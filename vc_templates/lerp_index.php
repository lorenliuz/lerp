...
global $lerp_post_types, $lerp_vc_index;
$lerp_vc_index = true;
...
if ( isset($lerp_post_types) && !empty($lerp_post_types) ) {
...
$loop_parse = lerp_parse_loop_data($loop);
...
foreach ( $post_types as $key => $value ) {
    ...
    if ( isset(${$value . '_items'}) )
        $post_blocks['lerp_' . $value] = lerp_flatArray(vc_sorted_list_parse_value(${$value . '_items'}));
}
...
?>
    <div<?php if ( $index_type === 'isotope' ) echo ' id="' . esc_attr($el_id) . '"'; ?>
            class="<?php echo esc_attr(trim(implode(' ', $main_container_classes))); ?>">
        <?php if ( $posts_counter > 0 && $index_type === 'isotope' ): ?>
            <?php if ( $filtering === 'yes' ) :
                ...
                if ( count($categories_array) > 1 || ($infinite === 'yes' && $filtering === 'yes') ) :
                    ...
                    if ( $filter_mobile === 'yes' ) echo ' mobile-hidden table-hidden';
                    if ( $filter_scroll === 'yes' ) echo ' filter-scroll';
                    if ( $inner_padding === 'yes' ) echo ' filters-inner-padding';
                    if ( $filter_sticky === 'yes' ) echo ' sticky-element'; ?>">
                      ...
                      <?php else: ?>
                                  <a href="<?php echo esc_url($current_url); ?>"
                                     class="<?php if ( !isset($_GET['ucat']) ) echo 'active'; ?>">
                      <?php endif; ?>
                                ...
                                <?php foreach ( $categories_array as $cat ):
                                    if ( $cat->taxonomy !== 'product_type' ): ?>
                                        <?php if ( ($infinite === 'yes' || $pagination !== 'yes' || $my_query->max_num_pages == 1) && !isset($_GET['ucat']) ) : ?>
                                            <li class="filter-cat-<?php echo esc_attr($cat->term_id); ?>"><span><a
                                                            href="#"
                                                            data-filter="grid-cat-<?php echo esc_attr($cat->term_id); ?>"
                                                            class="<?php if ( isset($_GET['ucat']) && $_GET['ucat'] == $cat->term_id ) echo 'active'; ?>"><?php echo esc_attr($cat->name) ?></a></span>
                                            </li>
                                        <?php else: ?>
                                            <li class="filter-cat-<?php echo esc_attr($cat->term_id); ?>"><span><a
                                                            href="<?php echo esc_url($current_url); ?>?id=<?php echo esc_attr($el_id); ?>&amp;ucat=<?php echo esc_attr($cat->term_id); ?>"
                                                            class="<?php if ( isset($_GET['ucat']) && $_GET['ucat'] == $cat->term_id ) echo 'active'; ?>"><?php echo esc_attr($cat->name) ?></a></span>
                                            </li>
                                        <?php endif; ?>
                                    <?php endif;
        ...
        <div class="<?php echo esc_attr(trim(implode(' ', $parent_container_classes))); ?>">
            <div<?php if ( $index_type === 'carousel' ) echo ' id="' . esc_attr($el_id) . '"'; ?>
                    class="<?php if ( $posts_counter > 0 ) echo esc_attr(trim(implode(' ', $container_classes))); ?>" <?php echo implode(' ', $div_data_attributes); ?>>
                <?php
                $i_matrix = 0;
                if ( $posts_counter > 0 ):
                    ...
                    foreach ( $posts as $post ):
                        ...
                        $typeLayout = $post_blocks['lerp_' . str_replace('-', '_', $post->type)];
                        $single_layout = 'single_layout_' . $post->type . '_items';

                        if ( isset($item_prop[$single_layout]) ) {
                            $typeLayout = lerp_flatArray(vc_sorted_list_parse_value($item_prop[$single_layout]));
                        }
                        ...
                        echo lerp_create_single_block($block_data, $el_id, $style_preset, $typeLayout, $lightbox_classes, 'no');

                        $i_matrix++;

                    endforeach;
                else: ?>
                    <div class="tmb tmb-iso-w12 tmb-iso-h1"><p
                                class="t-entry-title"><?php esc_html_e("Nothing found.", "lerp") ?></p></div>
                <?php endif; ?>
            </div>
                    ...
                    <?php if ( $infinite === 'yes' && $pagination !== 'yes' && $my_query->max_num_pages != 1 && $paged < $my_query->max_num_pages ): ?>
                        <nav class="loadmore-button"<?php if ( $infinite_button !== 'yes' ) echo ' style="display: none;"'; ?>>
                            ...
                            if ( $infinite_button_text === '' ) {
                                $infinite_button_text = ($infinite_button === 'yes') ? esc_html__('Load more', 'lerp') : esc_html__('Loading…', 'lerp');
                            }
                            ...
                            // Hover effect
                            $infinite_hover_fx = $infinite_hover_fx == '' ? ot_get_option('_lerp_button_hover') : $infinite_hover_fx;
                            ...
                            echo lerp_remove_wpautop($load_more_button);
                            ?>
                        </nav>
                    <?php else:
                        if ( $pagination === 'yes' ):
