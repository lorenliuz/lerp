...
$css_class = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'lerp-twentytwenty ' . $el_class, $this->settings['base'], $atts);

if ( $media_before === '' || $media_after === '' ) return;

$media_attributes_before = lerp_get_media_info($media_before);
...
$resized_image_before = lerp_resize_image($media_attributes_before->id, $media_attributes_before->guid, $media_attributes_before->path, $media_before_metavalue['width'], $media_before_metavalue['height'], 12, null, false);
$img_data_before = '';
...
$media_attributes_after = lerp_get_media_info($media_after);
...
$resized_image_after = lerp_resize_image($media_attributes_after->id, $media_attributes_after->guid, $media_attributes_after->path, $media_after_metavalue['width'], $media_after_metavalue['height'], 12, null, false);
$img_data_after = '';
if ( $adaptive_images === 'on' && $adaptive_images_async === 'on' ) {
...
<div class="<?php echo esc_attr($css_class); ?>">
    <div class="twentytwenty-container">
        <img src="<?php echo esc_url($resized_image_before['url']); ?>"
             width="<?php echo esc_attr($resized_image_before['width']); ?>"
             height="<?php echo esc_attr($resized_image_before['height']); ?>"<?php echo $img_class_before . $img_data_before; ?>>
        <img src="<?php echo esc_url($resized_image_after['url']); ?>"
             width="<?php echo esc_attr($resized_image_after['width']); ?>"
             height="<?php echo esc_attr($resized_image_after['height']); ?>"<?php echo $img_class_after . $img_data_after; ?>>
    </div>
</div>