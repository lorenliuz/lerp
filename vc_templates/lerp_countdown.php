...
$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'lerp-countdown', $this->settings['base'], $atts );
...
$output .= '<div class="lerp-wrapper '.esc_attr($css_class).'" '.implode(' ', $div_data_attributes).'>';
$output .= '<p>';
$output .= '<span class="countdown '.esc_attr(trim(implode(' ', $classes))).'" data-countdown="'.esc_attr($date).'">000 '.esc_html__('days','lerp').' 00 '.esc_html__('hours','lerp').' 00 '.esc_html__('minutes','lerp').' 00 '.esc_html__('seconds','lerp').'</span>';
$output .= '</p>';
