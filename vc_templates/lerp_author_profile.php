...
extract(shortcode_atts(array(
    ...
    'social' => 'yes',
    'display_button' => '',
    'button_content' => esc_html__('All author posts', 'lerp'),
    'url' => '',
    'button_link' => '',
    'link' => '',
...
$a_title = sprintf(esc_html__('%1$s post page', 'lerp'), $user_name);
$a_target = '_self';
...
if ( $avatar !== 'hide' ) {
    ...
    $media_html = lerp_create_single_block($block_data, 'single-' . $lbox_id, 'masonry', $layout, '', '');

    $media_string = '<div class="lerp-single-media-wrapper single-advanced">' . $media_html . '</div>';

    $avatar_output = '<div class="lerp-avatar-wrapper single-media lerp-single-media" style="width: ' . esc_attr($actual_width) . '">';
...
if ( $user_bio !== '' && $author_bio === 'yes' ) {
    ...
    $content_out .= '<div class="author-profile-bio ' . esc_attr($sub_class) . '">' . lerp_remove_wpautop($user_bio, true) . '</div>';
}
...
if ( $social === 'yes' ) {
    ...
    $content_out .= lerp_get_allowed_contact_methods($user_id);
...
if ( $display_button === 'yes' ) {
    ...
    $button_data_attributes = array_map(function ($v, $k) {
        return $k . '="' . $v . '"';
    }, $button_data, array_keys($button_data));
