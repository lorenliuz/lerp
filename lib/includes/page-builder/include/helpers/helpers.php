...
// Check if this file is loaded in page_builder
	...
	function vc_siteAttachedImages( $att_ids = array() ) {
		...
		foreach ( $media_images as $image_post ) {
			...
			$output .= '<li' . $class . '>
						...
						<span class="img-added">' . __( 'Added', 'page_builder' ) . '</span>
					</li>';
		}
 ...
 * Used in wp-content/plugins/page_builder/include/classes/core/class-vc-base.php\Vc_Base\bodyClass
 *
 * @param $classes
 *
 * @since 4.2
 * @return array
 */
function page_builder_body_class( $classes ) {
...
function vc_get_term_object( $term ) {
	...
	return array(
		...
		'group' => isset( $vc_taxonomies_types[ $term->taxonomy ], $vc_taxonomies_types[ $term->taxonomy ]->labels, $vc_taxonomies_types[ $term->taxonomy ]->labels->name ) ? $vc_taxonomies_types[ $term->taxonomy ]->labels->name : __( 'Taxonomies', 'page_builder' ),
	);
}
