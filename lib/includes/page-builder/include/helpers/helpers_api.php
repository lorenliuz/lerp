...
function vc_map( $attributes ) {
	if ( ! isset( $attributes['base'] ) ) {
		trigger_error( __( 'Wrong vc_map object. Base attribute is required', 'page_builder' ), E_USER_ERROR );
		die();
	}
...
function vc_map_add_css_animation( $label = true ) {
	$data = array(
		...
		'heading' => __( 'CSS Animation', 'page_builder' ),
		...
		'settings' => array(
			...
			'custom' => array(
				array(
					'label' => __( 'Default', 'page_builder' ),
					'values' => array(
						__( 'Top to bottom', 'page_builder' ) => 'top-to-bottom',
						__( 'Bottom to top', 'page_builder' ) => 'bottom-to-top',
						__( 'Left to right', 'page_builder' ) => 'left-to-right',
						__( 'Right to left', 'page_builder' ) => 'right-to-left',
						__( 'Appear from center', 'page_builder' ) => 'appear',
					),
				),
			),
		),
		'description' => __( 'Select type of animation for element to be animated when it "enters" the browsers viewport (Note: works only in modern browsers).', 'page_builder' ),
	);
