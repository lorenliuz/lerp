...
function vc_textarea_html_form_field( $settings, $value ) {
	...
	if ( false !== $vc_html_editor_already_is_use ) {
		...
		$output .= '<div class="updated"><p>'
		           . sprintf( __( 'Field type is changed from "textarea_html" to "textarea", because it is already used by %s field. Textarea_html field\'s type can be used only once per shortcode.', 'page_builder' ), $vc_html_editor_already_is_use )
		           . '</p></div>';
