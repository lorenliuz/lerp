...
class Vc_Column_Offset {
	...
	public function __construct( $settings, $value ) {
		...
		$this->column_width_list = array(
			__( '1 column - 1/12', 'page_builder' ) => '1',
			__( '2 columns - 1/6', 'page_builder' ) => '2',
			__( '3 columns - 1/4', 'page_builder' ) => '3',
			__( '4 columns - 1/3', 'page_builder' ) => '4',
			__( '5 columns - 5/12', 'page_builder' ) => '5',
			__( '6 columns - 1/2', 'page_builder' ) => '6',
			__( '7 columns - 7/12', 'page_builder' ) => '7',
			__( '8 columns - 2/3', 'page_builder' ) => '8',
			__( '9 columns - 3/4', 'page_builder' ) => '9',
			__( '10 columns - 5/6', 'page_builder' ) => '10',
			__( '11 columns - 11/12', 'page_builder' ) => '11',
			__( '12 columns - 1/1', 'page_builder' ) => '12',
		);
	}
	...
	public function sizeControl( $size ) {
		if ( 'sm' === $size ) {
			return '<span class="vc_description">' . __( 'Default value from width attribute', 'page_builder' ) . '</span>';
		}
		$empty_label = 'xs' === $size ? '' : __( 'Inherit from smaller', 'page_builder' );
	...
	public function offsetControl( $size ) {
		...
		$empty_label = 'xs' === $size ? __( 'No offset', 'page_builder' ) : __( 'Inherit from smaller', 'page_builder' );
		$output = '<select name="vc_' . $size . '_offset_size" class="vc_column_offset_field" data-type="offset-' . $size . '">' . '<option value="" style="color: #ccc;">' . $empty_label . '</option>' . ( 'xs' === $size ? '' : '<option value="' . $prefix . '0" style="color: #ccc;"' . ( in_array( $prefix . '0', $this->data ) ? ' selected="true"' : '' ) . '>' . __( 'No offset', 'page_builder' ) . '</option>' );
		foreach ( $this->column_width_list as $label => $index ) {
			$value = $prefix . $index;
			$output .= '<option value="' . $value . '"' . ( in_array( $value, $this->data ) ? ' selected="true"' : '' ) . '>' . $label . '</option>';
		}
