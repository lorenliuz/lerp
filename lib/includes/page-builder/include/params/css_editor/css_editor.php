	...
	class WPBakeryVisualComposerCssEditor {
		...
		function render() {
			...
			$output .= '<div class="vc_col-xs-5 vc_settings">'
			           ...
			           . '    <label>' . __( 'Box controls', 'page_builder' ) . '</label>'
			           . '    <label class="vc_checkbox"><input type="checkbox" name="simply" class="vc_simplify" value=""> ' . __( 'Simplify controls', 'page_builder' ) . '</label>'
			           . '</div>';
		...
		function getBackgroundImageControl() {
			return apply_filters( 'vc_css_editor_background_image_control', '<ul class="vc_image">'
				. '</ul>'
			. '<a href="#" class="vc_add-image"><i class="vc-composer-icon vc-c-icon-add"></i>' . __( 'Add image', 'page_builder' ) . '</a>' );
		}
		...
		function getBorderStyleOptions() {
			$output = '<option value="">' . __( 'Theme defaults', 'page_builder' ) . '</option>';
			...
			foreach ( $styles as $style ) {
				$output .= '<option value="' . $style . '">' . __( ucfirst( $style ), 'page_builder' ) . '</option>';
			}
		...
		function getBackgroundStyleOptions() {
			...
			$styles = apply_filters( 'vc_css_editor_background_style_options_data', array(
				__( 'Cover', 'page_builder' ) => 'cover',
				__( 'Contain', 'page_builder' ) => 'contain',
				__( 'No Repeat', 'page_builder' ) => 'no-repeat',
				__( 'Repeat', 'page_builder' ) => 'repeat',
			) );
