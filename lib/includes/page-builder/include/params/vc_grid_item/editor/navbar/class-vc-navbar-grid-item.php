...
class Vc_Navbar_Grid_Item extends Vc_Navbar {
	...
	public function getControlPreviewTemplate() {
		return '<li class="vc_pull-right">'
		       . '<a href="#" class="vc_btn vc_btn-grey vc_btn-sm vc_navbar-btn" data-vc-navbar-control="preview">' . __( 'Preview', 'page_builder' ) . '</a>'
		       . '</li>';
	}
	...
	public function getControlEdit() {
		return '<li class="vc_pull-right">'
		       ...
		       . __( 'Grid element settings', 'page_builder' ) . '"><i class="vc-composer-icon vc-c-icon-cog"></i>'
		       . '</a>'
		       . '</li>';
	}
	...
	public function getControlPreviewItemWidth() {
		...
		for ( $i = 1; $i <= 12; $i ++ ) {
			$output .= '<option value="' . esc_attr( $i ) . '">' . sprintf( __( '%s/12 width', 'page_builder' ), $i ) . '</option>';
		}
		$output .= '</select></li>';
