...
class Vc_Teaser_Box {
	...
	public function jsComposerEditPage() {
		...
		foreach ( $pt_array as $pt ) {
			add_meta_box( 'vc_teaser', __( 'VC: Custom Teaser', 'page_builder' ), array(
				$this,
				'outputTeaser',
			), $pt, 'side' );
		}
	...
	public function outputTeaser() {
		...
		wp_localize_script( 'wpb_jscomposer_teaser_js', 'i18nVcTeaser', array(
			'empty_title' => __( 'Empty title', 'page_builder' ),
			'text_label' => __( 'Text', 'page_builder' ),
			'image_label' => __( 'Image', 'page_builder' ),
			'title_label' => __( 'Title', 'page_builder' ),
			'link_label' => __( 'Link', 'page_builder' ),
			'text_text' => __( 'Text', 'page_builder' ),
			'text_excerpt' => __( 'Excerpt', 'page_builder' ),
			'text_custom' => __( 'Custom', 'page_builder' ),
			'image_featured' => __( 'Featered', 'page_builder' ),
			'image_custom' => __( 'Custom', 'page_builder' ),
			'link_label_text' => __( 'Link text', 'page_builder' ),
			'no_link' => __( 'No link', 'page_builder' ),
			'link_post' => __( 'Link to post', 'page_builder' ),
			'link_big_image' => __( 'Link to big image', 'page_builder' ),
			'add_custom_image' => __( 'Add custom image', 'page_builder' ),
		) );
		$output = '<div class="vc_teaser-switch"><label><input type="checkbox" name="' . self::$meta_data_name . '[enable]" value="1" id="vc_teaser-checkbox"' . ( '1' === $this->getTeaserData( 'enable' ) ? ' checked="true"' : '' ) . '> ' . __( 'Enable custom teaser', 'page_builder' ) . '</label></div>';
		...
		$output .= '<p>' . __( 'Customize teaser block design to overwrite default settings used in "Carousel" content element.', 'page_builder' ) . '</p>';
		$output .= '</div>';
