...
class Vc_Updater {
	...
	public function preUpgradeFilter( $reply, $package, $updater ) {
		...
		if ( ! $res ) {
			return new WP_Error( 'no_credentials', __( "Error! Can't connect to filesystem", 'page_builder' ) );
		}
		...
		if ( ! vc_license()->isActivated() ) {
			...
			return new WP_Error( 'no_credentials', __( 'To receive automatic updates license activation is required. Please visit <a href="' . $url . '' . '" target="_blank">Settings</a> to activate your WPBakery Page Builder.', 'page_builder' ) . ' ' . sprintf( ' <a href="http://go.wpbakery.com/faq-update-in-theme" target="_blank">%s</a>', __( 'Got WPBakery Page Builder in theme?', 'page_builder' ) ) );
		}

		$updater->strings['downloading_package_url'] = __( 'Getting download link...', 'page_builder' );
		$updater->skin->feedback( 'downloading_package_url' );

		$response = $this->getDownloadUrl();

		if ( ! $response ) {
			return new WP_Error( 'no_credentials', __( 'Download link could not be retrieved', 'page_builder' ) );
		}
		...
		$updater->strings['downloading_package'] = __( 'Downloading package...', 'page_builder' );
