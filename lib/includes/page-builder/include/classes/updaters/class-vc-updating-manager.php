...
class Vc_Updating_Manager {
	...
	public function addUpgradeMessageLink() {
		...
		if ( ! $is_activated ) {
			...
			$redirect = sprintf( '<a href="%s" target="_blank">%s</a>', $url, __( 'settings', 'page_builder' ) );

			echo sprintf( ' ' . __( 'To receive automatic updates license activation is required. Please visit %s to activate your WPBakery Page Builder.', 'page_builder' ), $redirect ) . sprintf( ' <a href="http://go.wpbakery.com/faq-update-in-theme" target="_blank">%s</a>', __( 'Got WPBakery Page Builder in theme?', 'page_builder' ) );
		}
	}
}
