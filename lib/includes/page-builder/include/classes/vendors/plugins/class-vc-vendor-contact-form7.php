...
class Vc_Vendor_ContactForm7 implements Vc_Vendor_Interface {
	...
	public function addShortcodeSettings( $tag ) {
		...
		if ( $cf7 ) {
			...
			$contact_forms[ __( 'No contact forms found', 'page_builder' ) ] = 0;
		}

		return array(
			'base' => $tag,
			'name' => __( 'Contact Form 7', 'page_builder' ),
			'icon' => 'icon-wpb-contactform7',
			'category' => __( 'Content', 'page_builder' ),
			'description' => __( 'Place Contact Form7', 'page_builder' ),
			'params' => array(
				array(
					'type' => 'dropdown',
					'heading' => __( 'Select contact form', 'page_builder' ),
					'param_name' => 'id',
					'value' => $contact_forms,
					'save_always' => true,
					'description' => __( 'Choose previously created contact form from the drop down list.', 'page_builder' ),
				),
				array(
					'type' => 'textfield',
					'heading' => __( 'Search title', 'page_builder' ),
					'param_name' => 'title',
					'admin_label' => true,
					'description' => __( 'Enter optional title to search if no ID selected or cannot find by ID.', 'page_builder' ),
				),
			),
		);
	}
