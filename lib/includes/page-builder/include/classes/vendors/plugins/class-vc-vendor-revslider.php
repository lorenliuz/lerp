...
class Vc_Vendor_Revslider implements Vc_Vendor_Interface {
	...
	public function mapShortcode( $revsliders = array() ) {
		vc_map( array(
			'base' => 'rev_slider_vc',
			'name' => __( 'Revolution Slider', 'page_builder' ),
			'icon' => 'icon-wpb-revslider',
			'category' => __( 'Content', 'page_builder' ),
			'description' => __( 'Place Revolution slider', 'page_builder' ),
			'params' => array(
				array(
					'type' => 'textfield',
				...
				array(
					'type' => 'dropdown',
					'heading' => __( 'Revolution Slider', 'page_builder' ),
					'param_name' => 'alias',
					'admin_label' => true,
					'value' => $revsliders,
					'save_always' => true,
					'description' => __( 'Select your Revolution Slider.', 'page_builder' ),
				),
				array(
					'type' => 'textfield',
	...
	public function addShortcodeSettings( $tag ) {
		...
		if ( $arrSliders ) {
			...
			$revsliders[ __( 'No sliders found', 'page_builder' ) ] = 0;
		}
