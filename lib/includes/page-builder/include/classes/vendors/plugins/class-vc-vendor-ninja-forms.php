...
class Vc_Vendor_NinjaForms implements Vc_Vendor_Interface {
	...
	public function addShortcodeSettings( $tag ) {
		...
		return array(
			'base' => $tag,
			'name' => __( 'Ninja Forms', 'page_builder' ),
			'icon' => 'icon-wpb-ninjaforms',
			'category' => __( 'Content', 'page_builder' ),
			'description' => __( 'Place Ninja Form', 'page_builder' ),
			'params' => array(
				array(
					'type' => 'dropdown',
					'heading' => __( 'Select ninja form', 'page_builder' ),
					'param_name' => 'id',
					'value' => $ninja_forms,
					'save_always' => true,
					'description' => __( 'Choose previously created ninja form from the drop down list.', 'page_builder' ),
				),
			),
		);
	}
