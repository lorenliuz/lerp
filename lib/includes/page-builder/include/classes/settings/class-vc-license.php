...
class Vc_License {
	 ...
	 * @var string
	 */
	static protected $license_key_option = 'page_builder_purchase_code';
	...
	function outputActivatedSuccess() {
		$this->outputNotice( __( 'WPBakery Page Builder successfully activated.', 'page_builder' ), true );
	}
	...
	function outputDeactivatedSuccess() {
		$this->outputNotice( __( 'WPBakery Page Builder successfully deactivated.', 'page_builder' ), true );
	}
	...
	function finishActivationDeactivation( $activation, $user_token ) {
		if ( ! $this->isValidToken( $user_token ) ) {
			$this->showError( __( 'Token is not valid or has expired', 'page_builder' ) );
		...
		if ( is_wp_error( $response ) ) {
			$this->showError( __( sprintf( '%s. Please try again.', $response->get_error_message() ), 'page_builder' ) );

			return false;
		}

		if ( 200 !== $response['response']['code'] ) {
			$this->showError( __( sprintf( 'Server did not respond with OK: %s', $response['response']['code'] ), 'page_builder' ) );
		...
		if ( ! $json || ! isset( $json['status'] ) ) {
			$this->showError( __( 'Invalid response structure. Please contact us for support.', 'page_builder' ) );

			return false;
		}

		if ( ! $json['status'] ) {
			$this->showError( __( 'Something went wrong. Please contact us for support.', 'page_builder' ) );
	...
	public function checkLicenseKeyFromRemote() {
		...
		if ( ! $this->isValid( $license_key ) ) {
			$response = array(
				'status' => false,
				'error' => __( 'Invalid license key', 'page_builder' ),
			);
		} else {
	...
	public function adminNoticeLicenseActivation() {
		if ( vc_is_network_plugin() ) {
			update_site_option( 'wpb_page_builder_license_activation_notified', 'yes' );
		} else {
		...
		<?php
		echo '<div class="updated vc_license-activation-notice" id="vc_license-activation-notice"><p>' . sprintf( __( 'Hola! Would you like to receive automatic updates and unlock premium support? Please <a href="%s">activate your copy</a> of WPBakery Page Builder.', 'page_builder' ), wp_nonce_url( $redirect ) ) . '</p>' . '<button type="button" class="notice-dismiss vc-notice-dismiss"><span class="screen-reader-text">' . __( 'Dismiss this notice.' ) . '</span></button></div>';
	}
