...
if ( ! class_exists( 'Vc_Automapper' ) ) {
	...
	class Vc_Automapper {
		...
		public function __construct() {
			$this->title = __( 'Shortcode Mapper', 'page_builder' );
		}
		...
		public function renderHtml() {
			...
			?>
			<div class="tab_intro">
				<p><?php _e( 'WPBakery Page Builder Shortcode Mapper adds custom 3rd party vendors shortcodes to the list of WPBakery Page Builder content elements menu (Note: to map shortcode it needs to be installed on site).', 'page_builder' ) ?></p>
			</div>
			<div class="vc_automapper-toolbar">
				<a href="#" class="button button-primary"
					id="vc_automapper-add-btn"><?php _e( 'Map Shortcode', 'page_builder' ) ?></a>
			</div>
		...
		public function renderMapFormTpl() {
			...
			<script type="text/html" id="vc_automapper-add-form-tpl">
				<label for="vc_atm-shortcode-string"
					class="vc_info"><?php _e( 'Shortcode string', 'page_builder' ) ?></label>

				<div class="vc_wrapper">
					<div class="vc_string">
						<input id="vc_atm-shortcode-string"
							placeholder="<?php _e( 'Please enter valid shortcode', 'page_builder' ) ?>"
							type="text" class="vc_atm-string">
					</div>
					<div class="vc_buttons">
						<a href="#" id="vc_atm-parse-string"
							class="button button-primary vc_parse-btn"><?php _e( 'Parse Shortcode', 'page_builder' ) ?></a>
						<a href="#" class="button vc_atm-cancel"><?php _e( 'Cancel', 'page_builder' ) ?></a>
					</div>
				</div>
				<span
					class="description"><?php _e( 'Enter valid shortcode (Example: [my_shortcode first_param="first_param_value"]My shortcode content[/my_shortcode]).', 'page_builder' ) ?></span>
			</script>
				...
				<div class="widget-top">
					<div class="widget-title-action">
						...
						<a class="widget-control-edit hide-if-js">
							<span class="edit vc_automapper-edit-btn"><?php _e( 'Edit', 'page_builder' ) ?></span>
							<span class="add vc_automapper-delete-btn"><?php _e( 'Add', 'page_builder' ) ?></span>
							<span class="screen-reader-text"><?php _e( 'Search', 'page_builder' ) ?></span>
						</a>
					</div>
			...
			<script type="text/html" id="vc_automapper-form-tpl">
				...
				<div class="vc_wrapper">
					<h4 class="vc_h"><?php _e( 'General Information', 'page_builder' ) ?></h4>

					<div class="vc_field vc_tag">
						<label for="vc_atm-tag"><?php _e( 'Tag:', 'page_builder' ) ?></label>
						<input type="text" name="tag" id="vc_atm-tag" value="{{ tag }}">
					</div>
					<div class="vc_field vc_description">
						<label for="vc_atm-description"><?php _e( 'Description:', 'page_builder' ) ?></label>
						<textarea name="description" id="vc_atm-description">{{ description }}</textarea>
					</div>
					<div class="vc_field vc_category">
						<label for="vc_atm-category"><?php _e( 'Category:', 'page_builder' ) ?></label>
						<input type="text" name="category" id="vc_atm-category" value="{{ category }}">
						<span
							class="description"><?php _e( 'Comma separated categories names', 'page_builder' ) ?></span>
					</div>
					<div class="vc_field vc_is-container">
						<label for="vc_atm-is-container"><input type="checkbox" name="is_container"
								...
								value=""> <?php _e( 'Include content param into shortcode', 'page_builder' ) ?>
						</label>
					</div>
				</div>
				<div class="vc_line"></div>
				<div class="vc_wrapper">
					<h4 class="vc_h"><?php _e( 'Shortcode Parameters', 'page_builder' ) ?></h4>
					<a href="#" id="vc_atm-add-param"
						class="button vc_add-param">+ <?php _e( 'Add Param', 'page_builder' ) ?></a>

					<div class="vc_params" id="vc_atm-params-list"></div>
				</div>
				<div class="vc_buttons">
					<a href="#" id="vc_atm-save"
						class="button button-primary"><?php _e( 'Save Changes', 'page_builder' ) ?></a>
					<a href="#" class="button vc_atm-cancel"><?php _e( 'Cancel', 'page_builder' ) ?></a>
					<a href="#" class="button vc_atm-delete"><?php _e( 'Delete', 'page_builder' ) ?></a>
				</div>
			</script>
			<script type="text/html" id="vc_atm-form-param-tpl">
				<div class="vc_controls vc_controls-row vc_clearfix"><a
						...
						title="<?php _e( 'Drag row to reorder', 'page_builder' ) ?>" data-vc-control="move"><i
							...
							title="<?php _e( 'Delete this param', 'page_builder' ) ?>"><i class="vc-composer-icon vc-c-icon-delete_empty"></i></a></span>
				</div>
							...
							<div class="wpb_element_wrapper">
								<div class="vc_fields vc_clearfix">
									<div class="vc_param_name vc_param-field">
										<label><?php _e( 'Param name', 'page_builder' ) ?></label>
										<# if ( 'content' === param_name) { #>
											<span class="vc_content"><?php _e( 'Content', 'page_builder' ) ?></span>
											<input type="text" style="display: none;" name="param_name"
												value="{{ param_name }}"
												placeholder="<?php _e( 'Required value', 'page_builder' ) ?>"
												class="vc_param-name"
												data-system="true">
											<span class="description"
												style="display: none;"><?php _e( 'Use only letters, numbers and underscore.', 'page_builder' ) ?></span>
											<# } else { #>
												<input type="text" name="param_name" value="{{ param_name }}"
													placeholder="<?php _e( 'Required value', 'page_builder' ) ?>"
													class="vc_param-name">
												<span
													class="description"><?php _e( 'Please use only letters, numbers and underscore.', 'page_builder' ) ?></span>
												<# } #>
									</div>
									<div class="vc_heading vc_param-field">
										<label><?php _e( 'Heading', 'page_builder' ) ?></label>
										<input type="text" name="heading" value="{{ heading }}"
											placeholder="<?php _e( 'Input heading', 'page_builder' ) ?>"
										<# if ( 'hidden' === type) { #>
											disabled="disabled"
											<# } #>>
												<span
													class="description"><?php _e( 'Heading for field in shortcode edit form.', 'page_builder' ) ?></span>
									</div>
									<div class="vc_type vc_param-field">
										<label><?php _e( 'Field type', 'page_builder' ) ?></label>
										<select name="type">
											<option value=""><?php _e( 'Select field type', 'page_builder' ) ?></option>
											<option
												value="textfield"<?php echo '<# if (type === "textfield") { #> selected<# } #>' ?>><?php _e( 'Textfield', 'page_builder' ) ?></option>
											<option
												value="dropdown"<?php echo '<# if (type === "dropdown") { #> selected<# } #>' ?>><?php _e( 'Dropdown', 'page_builder' ) ?></option>
											<option
												value="textarea"<?php echo '<# if(type==="textarea") { #> selected="selected"<# } #>' ?>><?php _e( 'Textarea', 'page_builder' ) ?></option>
											<# if ( 'content' === param_name ) { #>
												<option
													value="textarea_html"<?php echo '<# if (type === "textarea_html") { #> selected<# } #>' ?>><?php _e( 'Textarea HTML', 'page_builder' ) ?></option>
												<# } #>
													<option
														value="hidden"<?php echo '<# if (type === "hidden") { #> selected<# } #>' ?>><?php _e( 'Hidden', 'page_builder' ) ?></option>

										</select>
										<span
											class="description"><?php _e( 'Field type for shortcode edit form.', 'page_builder' ) ?></span>
									</div>
									<div class="vc_value vc_param-field">
										<label><?php _e( 'Default value', 'page_builder' ) ?></label>
										<input type="text" name="value" value="{{ value }}" class="vc_param-value">
										<span
											class="description"><?php _e( 'Default value or list of values for dropdown type (Note: separate by comma).', 'page_builder' ) ?></span>
									</div>
									<div class="description vc_param-field">
										<label><?php _e( 'Description', 'page_builder' ) ?></label>
										...
										<# if ( 'hidden' === type ) { #>
											...
											<# } #> >{{ description
												...
												<span
													class="description"><?php _e( 'Enter description for parameter.', 'page_builder' ) ?></span>
									</div>
								</div>
							</div>
						</div>
		...
		public function renderTemplates() {
			...
			<script type="text/html" id="vc_automapper-item-tpl">
				...
				<span class="vc_automapper-item-controls">
					<a href="#" class="vc_automapper-delete-btn" title="<?php _e( 'Delete', 'page_builder' ) ?>"></a>
					<a href="#" class="vc_automapper-edit-btn" title="<?php _e( 'Edit', 'page_builder' ) ?>"></a>
				</span>
			</script>
			<?php
