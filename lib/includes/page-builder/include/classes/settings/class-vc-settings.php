...
class Vc_Settings {
	 ...
	 * @var string
	 */
	protected $option_group = 'wpb_page_builder_settings';
	/**
	 * @var string
	 */
	...
	public function setTabs() {
		...
		if ( $this->showConfigurationTabs() ) {
			$this->tabs['vc-general'] = __( 'General Settings', 'page_builder' );
			if ( ! vc_is_as_theme() || apply_filters( 'vc_settings_page_show_design_tabs', false ) ) {
				$this->tabs['vc-color'] = __( 'Design Options', 'page_builder' );
				$this->tabs['vc-custom_css'] = __( 'Custom CSS', 'page_builder' );
			}
		}

		if ( ! vc_is_network_plugin() || ( vc_is_network_plugin() && is_network_admin() ) ) {
			if ( ! vc_is_updater_disabled() ) {
				$this->tabs['vc-updater'] = __( 'Product License', 'page_builder' );
			}
		}
	...
	public function initAdmin() {
		...
		self::$color_settings = array(
			array( 'vc_color' => array( 'title' => __( 'Main accent color', 'page_builder' ) ) ),
			array( 'vc_color_hover' => array( 'title' => __( 'Hover color', 'page_builder' ) ) ),
			array( 'vc_color_call_to_action_bg' => array( 'title' => __( 'Call to action background color', 'page_builder' ) ) ),
			array( 'vc_color_google_maps_bg' => array( 'title' => __( 'Google maps background color', 'page_builder' ) ) ),
			array( 'vc_color_post_slider_caption_bg' => array( 'title' => __( 'Post slider caption background color', 'page_builder' ) ) ),
			array( 'vc_color_progress_bar_bg' => array( 'title' => __( 'Progress bar background color', 'page_builder' ) ) ),
			array( 'vc_color_separator_border' => array( 'title' => __( 'Separator border color', 'page_builder' ) ) ),
			array( 'vc_color_tab_bg' => array( 'title' => __( 'Tabs navigation background color', 'page_builder' ) ) ),
			array( 'vc_color_tab_bg_active' => array( 'title' => __( 'Active tab background color', 'page_builder' ) ) ),
		);
		self::$defaults = array(
			'vc_color' => '#f7f7f7',
		 ...
		 */
		add_action( 'update_option_wpb_js_compiled_page_builder_less', array(
			$this,
			'buildCustomColorCss',
		) );
		 ...
		 * @since 4.5 used to call update file once option is changed
		 */
		add_action( 'add_option_wpb_js_compiled_page_builder_less', array(
			$this,
			'buildCustomColorCss',
		) );
		...
		$this->addField( $tab, __( 'Disable responsive content elements', 'page_builder' ), 'not_responsive_css', array(
			$this,
		...
		$this->addField( $tab, __( 'Google fonts subsets', 'page_builder' ), 'google_fonts_subsets', array(
			$this,
		...
		// Use custom checkbox
		$this->addField( $tab, __( 'Use custom design options', 'page_builder' ), 'use_custom', array(
			$this,
		...
		// Margin
		$this->addField( $tab, __( 'Elements bottom margin', 'page_builder' ), 'margin', array(
			$this,
		...
		// Gutter
		$this->addField( $tab, __( 'Grid gutter width', 'page_builder' ), 'gutter', array(
			$this,
		...
		$this->addField( $tab, __( 'Mobile screen width', 'page_builder' ), 'responsive_max', array(
			$this,
		...
		$this->addField( $tab, false, 'compiled_page_builder_less', array(
			$this,
			'sanitize_compiled_page_builder_less_callback',
		), array(
			$this,
			'compiled_page_builder_less_callback',
		) );
		...
		$tab = 'custom_css';
		$this->addSection( $tab );
		$this->addField( $tab, __( 'Paste your CSS code', 'page_builder' ), 'custom_css', array(
			$this,
	...
	public function restoreColor() {
		...
		delete_option( self::$field_prefix . 'compiled_page_builder_less' );
	...
	function adminLoad() {
		wp_register_script( 'wpb_page_builder_settings', vc_asset_url( 'js/dist/settings.min.js' ), array(), WPB_VC_VERSION, true );
		wp_enqueue_style( 'page_builder_settings', vc_asset_url( 'css/page_builder_settings.min.css' ), false, WPB_VC_VERSION );
		wp_enqueue_script( 'backbone' );
		wp_enqueue_script( 'shortcode' );
		wp_enqueue_script( 'underscore' );
		wp_enqueue_script( 'jquery-ui-accordion' );
		wp_enqueue_script( 'jquery-ui-sortable' );
		wp_enqueue_script( 'wpb_page_builder_settings' );
		$this->locale = array(
			'are_you_sure_reset_css_classes' => __( 'Are you sure you want to reset to defaults?', 'page_builder' ),
			'are_you_sure_reset_color' => __( 'Are you sure you want to reset to defaults?', 'page_builder' ),
			'saving' => __( 'Saving...', 'page_builder' ),
			'save' => __( 'Save Changes', 'page_builder' ),
			'saved' => __( 'Design Options successfully saved.', 'page_builder' ),
			'save_error' => __( 'Design Options could not be saved', 'page_builder' ),
			'form_save_error' => __( 'Problem with AJAX request execution, check internet connection and try again.', 'page_builder' ),
			'are_you_sure_delete' => __( 'Are you sure you want to delete this shortcode?', 'page_builder' ),
			'are_you_sure_delete_param' => __( "Are you sure you want to delete the shortcode's param?", 'page_builder' ),
			'my_shortcodes_category' => __( 'My shortcodes', 'page_builder' ),
			'error_shortcode_name_is_required' => __( 'Shortcode name is required.', 'page_builder' ),
			'error_enter_valid_shortcode_tag' => __( 'Please enter valid shortcode tag.', 'page_builder' ),
			'error_enter_required_fields' => __( 'Please enter all required fields for params.', 'page_builder' ),
			'new_shortcode_mapped' => __( 'New shortcode mapped from string!', 'page_builder' ),
			'shortcode_updated' => __( 'Shortcode updated!', 'page_builder' ),
			'error_content_param_not_manually' => __( 'Content param can not be added manually, please use checkbox.', 'page_builder' ),
			'error_param_already_exists' => __( 'Param %s already exists. Param names must be unique.', 'page_builder' ),
			'error_wrong_param_name' => __( 'Please use only letters, numbers and underscore for param name', 'page_builder' ),
			'error_enter_valid_shortcode' => __( 'Please enter valid shortcode to parse!', 'page_builder' ),

		);
		wp_localize_script( 'wpb_page_builder_settings', 'vcData', apply_filters( 'vc_global_js_data', array(
			'version' => WPB_VC_VERSION,
			'debug' => wpb_debug(),
		) ) );
		wp_localize_script( 'wpb_page_builder_settings', 'i18nLocaleSettings', $this->locale );
	}
	...
	public function custom_css_field_callback() {
		...
		echo '<p class="description indicator-hint">' . __( 'Add custom CSS code to the plugin without modifying files.', 'page_builder' ) . '</p>';
	}
	...
	public function not_responsive_css_field_callback() {
		...
		<label>
			...
			<?php _e( 'Disable', 'page_builder' ) ?>
		</label><br/>
		<p
				class="description indicator-hint"><?php _e( 'Disable content elements from "stacking" one on top other on small media screens (Example: mobile devices).', 'page_builder' ); ?></p>
		<?php
	}
	...
	public function google_fonts_subsets_callback() {
		...
		?>
		<p
				class="description indicator-hint"><?php _e( 'Select subsets for Google Fonts available to content elements.', 'page_builder' ); ?></p>
		<?php
	}
	...
	public function use_custom_callback() {
		...
		<label>
			...
			<?php _e( 'Enable', 'page_builder' ) ?>
		</label><br/>
		<p
				class="description indicator-hint"><?php _e( 'Enable the use of custom design options (Note: when checked - custom css file will be used).', 'page_builder' ); ?></p>
		<?php
	}
	...
	public function margin_callback() {
		...
		echo '<p class="description indicator-hint css-control">' . __( 'Change default vertical spacing between content elements (Example: 20px).', 'page_builder' ) . '</p>';
	}
	...
	public function gutter_callback() {
		...
		echo '<p class="description indicator-hint css-control">' . __( 'Change default horizontal spacing between columns, enter new value in pixels.', 'page_builder' ) . '</p>';
	}
	...
	public function responsive_max_callback() {
		...
		$value = ( $value = get_option( self::$field_prefix . $field ) ) ? $value : $this->getDefault( $field );
		echo '<input type="text" name="' . self::$field_prefix . $field . '" value="' . $value . '" class="css-control"> px';
		echo '<p class="description indicator-hint css-control">' . __( 'By default content elements "stack" one on top other when screen size is smaller than 768px. Change the value to change "stacking" size.', 'page_builder' ) . '</p>';
	}

	/**
	 *
	 */
	public function compiled_page_builder_less_callback() {
		$field = 'compiled_page_builder_less';
	...
	public function setting_section_callback_function( $tab ) {
		if ( 'wpb_page_builder_settings_color' === $tab['id'] ) : ?>
			<div class="tab_intro">
				<p>
					<?php _e( 'Here you can tweak default WPBakery Page Builder content elements visual appearance. By default WPBakery Page Builder is using neutral light-grey theme. Changing "Main accent color" will affect all content elements if no specific "content block" related color is set.', 'page_builder' ) ?>
				</p>
			</div>
		<?php endif;
	}
	 ...
	 * @return mixed
	 */
	public function sanitize_compiled_page_builder_less_callback( $css ) {
		return $css;
	}
	...
	public function sanitize_margin_callback( $margin ) {
		...
		if ( ! preg_match( '/^\d+(px|%|em|pt){0,1}$/', $margin ) ) {
			add_settings_error( self::$field_prefix . 'margin', 1, __( 'Invalid Margin value.', 'page_builder' ), 'error' );
		}
	...
	public function sanitize_gutter_callback( $gutter ) {
		...
		if ( ! $this->_isGutterValid( $gutter ) ) {
			add_settings_error( self::$field_prefix . 'gutter', 1, __( 'Invalid Gutter value.', 'page_builder' ), 'error' );
		}
	...
	public function sanitize_responsive_max_callback( $responsive_max ) {
		if ( ! $this->_isNumberValid( $responsive_max ) ) {
			add_settings_error( self::$field_prefix . 'responsive_max', 1, __( 'Invalid "Responsive max" value.', 'page_builder' ), 'error' );
		}
	...
	public static function buildCustomColorCss() {
		 ...
		 *
		 */
		if ( false === ( $page_builder_upload_dir = self::checkCreateUploadDir( $wp_filesystem, 'use_custom', 'page_builder_front_custom.css' ) ) ) {
			return;
		}

		$filename = $page_builder_upload_dir . '/page_builder_front_custom.css';
		...
		$css_string = get_option( self::$field_prefix . 'compiled_page_builder_less' );
		if ( strlen( trim( $css_string ) ) > 0 ) {
			...
			if ( ! $wp_filesystem->put_contents( $filename, $css_string, FS_CHMOD_FILE ) ) {
				if ( is_wp_error( $wp_filesystem->errors ) && $wp_filesystem->errors->get_error_code() ) {
					add_settings_error( self::$field_prefix . 'main_color', $wp_filesystem->errors->get_error_code(), __( 'Something went wrong: page_builder_front_custom.css could not be created.', 'page_builder' ) . ' ' . $wp_filesystem->errors->get_error_message(), 'error' );
				} elseif ( ! $wp_filesystem->connect() ) {
					add_settings_error( self::$field_prefix . 'main_color', $wp_filesystem->errors->get_error_code(), __( 'page_builder_front_custom.css could not be created. Connection error.', 'page_builder' ), 'error' );
				} elseif ( ! $wp_filesystem->is_writable( $filename ) ) {
					add_settings_error( self::$field_prefix . 'main_color', $wp_filesystem->errors->get_error_code(), sprintf( __( 'page_builder_front_custom.css could not be created. Cannot write custom css to "%s".', 'page_builder' ), $filename ), 'error' );
				} else {
					add_settings_error( self::$field_prefix . 'main_color', $wp_filesystem->errors->get_error_code(), __( 'page_builder_front_custom.css could not be created. Problem with access.', 'page_builder' ), 'error' );
				}
				delete_option( self::$field_prefix . 'use_custom' );
				delete_option( self::$field_prefix . 'less_version' );
			}
	...
	public static function buildCustomCss() {
		 ...
		 * Building css file.
		 */
		if ( false === ( $page_builder_upload_dir = self::checkCreateUploadDir( $wp_filesystem, 'custom_css', 'custom.css' ) ) ) {
			return true;
		}

		$filename = $page_builder_upload_dir . '/custom.css';
		$css_string = '';
		...
		if ( ! $wp_filesystem->put_contents( $filename, $css_string, FS_CHMOD_FILE ) ) {
			if ( is_wp_error( $wp_filesystem->errors ) && $wp_filesystem->errors->get_error_code() ) {
				add_settings_error( self::$field_prefix . 'custom_css', $wp_filesystem->errors->get_error_code(), __( 'Something went wrong: custom.css could not be created.', 'page_builder' ) . $wp_filesystem->errors->get_error_message(), 'error' );
			} elseif ( ! $wp_filesystem->connect() ) {
				add_settings_error( self::$field_prefix . 'custom_css', $wp_filesystem->errors->get_error_code(), __( 'custom.css could not be created. Connection error.', 'page_builder' ), 'error' );
			} elseif ( ! $wp_filesystem->is_writable( $filename ) ) {
				add_settings_error( self::$field_prefix . 'custom_css', $wp_filesystem->errors->get_error_code(), __( 'custom.css could not be created. Cannot write custom css to "' . $filename . '".', 'page_builder' ), 'error' );
			} else {
				add_settings_error( self::$field_prefix . 'custom_css', $wp_filesystem->errors->get_error_code(), __( 'custom.css could not be created. Problem with access.', 'page_builder' ), 'error' );
			}
	...
	public static function checkCreateUploadDir( $wp_filesystem, $option, $filename ) {
		$page_builder_upload_dir = self::uploadDir();
		if ( ! $wp_filesystem->is_dir( $page_builder_upload_dir ) ) {
			if ( ! $wp_filesystem->mkdir( $page_builder_upload_dir, 0777 ) ) {
				add_settings_error( self::$field_prefix . $option, $wp_filesystem->errors->get_error_code(), __( sprintf( '%s could not be created. Not available to create page_builder directory in uploads directory (' . $page_builder_upload_dir . ').', $filename ), 'page_builder' ), 'error' );

				return false;
			}
		}

		return $page_builder_upload_dir;
	}
