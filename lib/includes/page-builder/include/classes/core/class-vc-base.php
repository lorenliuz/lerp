...
class Vc_Base {
	...
	public function frontCss() {
		...
		$front_css_file = vc_asset_url( 'css/page_builder/min.css' );
		$upload_dir = wp_upload_dir();
		$vc_upload_dir = vc_upload_dir();
		if ( '1' === vc_settings()->get( 'use_custom' ) && is_file( $upload_dir['basedir'] . '/' . $vc_upload_dir . '/page_builder_front_custom.css' ) ) {
			$front_css_file = $upload_dir['baseurl'] . '/' . $vc_upload_dir . '/page_builder_front_custom.css';
			$front_css_file = vc_str_remove_protocol( $front_css_file );
		}
		wp_register_style( 'page_builder_front', $front_css_file, array(), WPB_VC_VERSION );
		...
		if ( is_file( $upload_dir['basedir'] . '/' . $vc_upload_dir . '/custom.css' ) && filesize( $custom_css_path ) > 0 ) {
			...
			wp_register_style( 'page_builder_custom_css', $custom_css_url, array(), WPB_VC_VERSION );
		}
	...
	public function enqueueStyle() {
		...
		if ( $post && preg_match( '/vc_row/', $post->post_content ) ) {
			wp_enqueue_style( 'page_builder_front' );
		}
		wp_enqueue_style( 'page_builder_custom_css' );
	}
	...
	public function frontJsRegister() {
		...
		wp_register_script( 'wpb_composer_front_js', vc_asset_url( 'js/dist/page_builder_front.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
	...
	public function pluginActionLinks( $links, $file ) {
		if ( plugin_basename( vc_path_dir( 'APP_DIR', '/page_builder.php' ) ) == $file ) {
			$title = __( 'WPBakery Page Builder Settings', 'page_builder' );
			$html = esc_html__( 'Settings', 'page_builder' );
			if ( ! vc_user_access()->part( 'settings' )->can( 'vc-general-tab' )->get() ) {
				$title = __( 'About WPBakery Page Builder', 'page_builder' );
				$html = esc_html__( 'About', 'page_builder' );
			}
	...
	public function bodyClass( $classes ) {
		return page_builder_body_class( $classes );
	}
	...
	public function getEditorsLocale() {
		return array(
			'add_remove_picture' => __( 'Add/remove picture', 'page_builder' ),
			'finish_adding_text' => __( 'Finish Adding Images', 'page_builder' ),
			'add_image' => __( 'Add Image', 'page_builder' ),
			'add_images' => __( 'Add Images', 'page_builder' ),
			'settings' => __( 'Settings', 'page_builder' ),
			'main_button_title' => __( 'WPBakery Page Builder', 'page_builder' ),
			'main_button_title_backend_editor' => __( 'Backend Editor', 'page_builder' ),
			'main_button_title_frontend_editor' => __( 'Frontend Editor', 'page_builder' ),
			'main_button_title_revert' => __( 'Classic Mode', 'page_builder' ),
			'please_enter_templates_name' => __( 'Enter template name you want to save.', 'page_builder' ),
			'confirm_deleting_template' => __( 'Confirm deleting "{template_name}" template, press Cancel to leave. This action cannot be undone.', 'page_builder' ),
			'press_ok_to_delete_section' => __( 'Press OK to delete section, Cancel to leave', 'page_builder' ),
			'drag_drop_me_in_column' => __( 'Drag and drop me in the column', 'page_builder' ),
			'press_ok_to_delete_tab' => __( 'Press OK to delete "{tab_name}" tab, Cancel to leave', 'page_builder' ),
			'slide' => __( 'Slide', 'page_builder' ),
			'tab' => __( 'Tab', 'page_builder' ),
			'section' => __( 'Section', 'page_builder' ),
			'please_enter_new_tab_title' => __( 'Please enter new tab title', 'page_builder' ),
			'press_ok_delete_section' => __( 'Press OK to delete "{tab_name}" section, Cancel to leave', 'page_builder' ),
			'section_default_title' => __( 'Section', 'page_builder' ),
			'please_enter_section_title' => __( 'Please enter new section title', 'page_builder' ),
			'error_please_try_again' => __( 'Error. Please try again.', 'page_builder' ),
			'if_close_data_lost' => __( 'If you close this window all shortcode settings will be lost. Close this window?', 'page_builder' ),
			'header_select_element_type' => __( 'Select element type', 'page_builder' ),
			'header_media_gallery' => __( 'Media gallery', 'page_builder' ),
			'header_element_settings' => __( 'Element settings', 'page_builder' ),
			'add_tab' => __( 'Add tab', 'page_builder' ),
			'are_you_sure_convert_to_new_version' => __( 'Are you sure you want to convert to new version?', 'page_builder' ),
			'loading' => __( 'Loading...', 'page_builder' ),
			// Media editor
			'set_image' => __( 'Set Image', 'page_builder' ),
			'are_you_sure_reset_css_classes' => __( 'Are you sure that you want to remove all your data?', 'page_builder' ),
			'loop_frame_title' => __( 'Loop settings', 'page_builder' ),
			'enter_custom_layout' => __( 'Custom row layout', 'page_builder' ),
			'wrong_cells_layout' => __( 'Wrong row layout format! Example: 1/2 + 1/2 or span6 + span6.', 'page_builder' ),
			'row_background_color' => __( 'Row background color', 'page_builder' ),
			'row_background_image' => __( 'Row background image', 'page_builder' ),
			'column_background_color' => __( 'Column background color', 'page_builder' ),
			'column_background_image' => __( 'Column background image', 'page_builder' ),
			'guides_on' => __( 'Guides ON', 'page_builder' ),
			'guides_off' => __( 'Guides OFF', 'page_builder' ),
			'template_save' => __( 'New template successfully saved.', 'page_builder' ),
			'template_added' => __( 'Template added to the page.', 'page_builder' ),
			'template_added_with_id' => __( 'Template added to the page. Template has ID attributes, make sure that they are not used more than once on the same page.', 'page_builder' ),
			'template_removed' => __( 'Template successfully removed.', 'page_builder' ),
			'template_is_empty' => __( 'Template is empty: There is no content to be saved as a template.', 'page_builder' ),
			'template_save_error' => __( 'Error while saving template.', 'page_builder' ),
			'css_updated' => __( 'Page settings updated!', 'page_builder' ),
			'update_all' => __( 'Update all', 'page_builder' ),
			'confirm_to_leave' => __( 'The changes you made will be lost if you navigate away from this page.', 'page_builder' ),
			'inline_element_saved' => __( '%s saved!', 'page_builder' ),
			'inline_element_deleted' => __( '%s deleted!', 'page_builder' ),
			'inline_element_cloned' => __( '%s cloned. <a href="#" class="vc_edit-cloned" data-model-id="%s">Edit now?</a>', 'page_builder' ),
			'gfonts_loading_google_font_failed' => __( 'Loading Google Font failed', 'page_builder' ),
			'gfonts_loading_google_font' => __( 'Loading Font...', 'page_builder' ),
			'gfonts_unable_to_load_google_fonts' => __( 'Unable to load Google Fonts', 'page_builder' ),
			'no_title_parenthesis' => sprintf( '(%s)', __( 'no title', 'page_builder' ) ),
			'error_while_saving_image_filtered' => __( 'Error while applying filter to the image. Check your server and memory settings.', 'page_builder' ),
			'ui_saved' => sprintf( '<i class="vc-composer-icon vc-c-icon-check"></i> %s', __( 'Saved!', 'page_builder' ) ),
			'ui_danger' => sprintf( '<i class="vc-composer-icon vc-c-icon-close"></i> %s', __( 'Failed to Save!', 'page_builder' ) ),
			'delete_preset_confirmation' => __( 'You are about to delete this preset. This action can not be undone.', 'page_builder' ),
			'ui_template_downloaded' => __( 'Downloaded', 'page_builder' ),
			'ui_template_update' => __( 'Update', 'page_builder' ),
			'ui_templates_failed_to_download' => __( 'Failed to download template', 'page_builder' ),
			'preset_removed' => __( 'Element successfully removed.', 'page_builder' ),
		);
	}
}
