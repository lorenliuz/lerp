...
class WPBMap {
	...
	public static function map( $tag, $attributes ) {
		...
		if ( ! self::$is_init ) {
			if ( empty( $attributes['name'] ) ) {
				trigger_error( sprintf( __( 'Wrong name for shortcode:%s. Name required', 'page_builder' ), $tag ) );
			} elseif ( empty( $attributes['base'] ) ) {
				trigger_error( sprintf( __( 'Wrong base for shortcode:%s. Base required', 'page_builder' ), $tag ) );
			} else {
	...
	public static function getParam( $tag, $param_name ) {
		if ( ! isset( self::$sc[ $tag ] ) ) {
			return trigger_error( sprintf( __( 'Wrong name for shortcode:%s. Name required', 'page_builder' ), $tag ) );
		}
	...
	public static function addParam( $name, $attribute = array() ) {
		...
		if ( ! isset( self::$sc[ $name ] ) ) {
			trigger_error( sprintf( __( 'Wrong name for shortcode:%s. Name required', 'page_builder' ), $name ) );
		} elseif ( ! isset( $attribute['param_name'] ) ) {
			trigger_error( sprintf( __( "Wrong attribute for '%s' shortcode. Attribute 'param_name' required", 'page_builder' ), $name ) );
		} else {
	...
	public static function mutateParam( $name, $attribute = array() ) {
		...
		if ( ! isset( self::$sc[ $name ] ) ) {
			return trigger_error( sprintf( __( 'Wrong name for shortcode:%s. Name required', 'page_builder' ), $name ) );
		} elseif ( ! isset( $attribute['param_name'] ) ) {
			trigger_error( sprintf( __( "Wrong attribute for '%s' shortcode. Attribute 'param_name' required", 'page_builder' ), $name ) );
		} else {
	...
	public static function modify( $name, $setting_name, $value = '' ) {
		...
		if ( ! isset( self::$sc[ $name ] ) ) {
			...
			return trigger_error( sprintf( __( "Wrong setting_name for shortcode:%s. Base can't be modified.", 'page_builder' ), $name ) );
		}
