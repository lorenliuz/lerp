...
class Vc_Templates_Panel_Editor {
		...
		if ( 'my_templates' === $category['category'] ) {
			...
			if ( vc_user_access()->part( 'templates' )->checkStateAny( true, null )->get() ) {
				...
				<div class="vc_column vc_col-sm-12" data-vc-hide-on-search="true">
					<div class="vc_element_label">' . esc_html( __( 'Save current layout as a template', 'page_builder' ) ) . '</div>
					<div class="vc_input-group">
						<input name="padding" data-js-element="vc-templates-input" class="vc_form-control wpb-textinput vc_panel-templates-name" type="text" value="" placeholder="' . esc_attr__( 'Template name', 'page_builder' ) . '" data-vc-disable-empty="#vc_ui-save-template-btn">
						<span class="vc_input-group-btn">
							<button class="vc_general vc_ui-button vc_ui-button-size-sm vc_ui-button-action vc_ui-button-shape-rounded vc_template-save-btn" id="vc_ui-save-template-btn" disabled data-vc-ui-element="button-save">' . esc_html( __( 'Save Template', 'page_builder' ) ) . '</button>
						</span>
					</div>
					<span class="vc_description">' . esc_html( __( 'Save layout and reuse it on different sections of this site.', 'page_builder' ) ) . '</span>
				</div>';
			}
	...
	public function renderTemplateWindowMyTemplates( $template_name, $template_data ) {
		...
		$preview_template_title = esc_attr__( 'Preview template', 'page_builder' );
		$add_template_title = esc_attr__( 'Add template', 'page_builder' );
		$deleteControlRender = '';
		if ( vc_user_access()->part( 'templates' )->checkStateAny( true, null )->get() ) {
			$delete_template_title = esc_attr__( 'Delete template', 'page_builder' );
...
HTML;
	...
	public function renderTemplatePreview() {
		...
		if ( ! isset( $template_id, $template_type ) || '' === $template_id || '' === $template_type ) {
			die( __( 'Error: wrong template id.', 'page_builder' ) );
		}
		WPBMap::addAllMappedShortcodes();
		if ( 'my_templates' === $template_type ) {
			$saved_templates = get_option( $this->option_name );
	...
	public function getAllTemplates() {
		...
		if ( apply_filters( 'vc_show_user_templates', true ) ) {
			...
			$arr_category = array(
				...
				'category_name' => __( 'My Templates', 'page_builder' ),
				'category_description' => __( 'Append previously saved template to the current layout.', 'page_builder' ),
				'category_weight' => 10,
			);
		...
		if ( ! empty( $default_templates ) ) {
			$arr_category = array(
				...
				'category_name' => __( 'Default Templates', 'page_builder' ),
				'category_description' => __( 'Append default template to the current layout.', 'page_builder' ),
				'category_weight' => 11,
			);
			$category_templates = array();
	...
	public function renderTemplateListItem( $template ) {
		$name = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'page_builder' ) );
