...
class Vc_Shortcode_Edit_Form implements Vc_Render {
	...
	public function renderFields() {
		...
		vc_user_access()->checkAdminNonce()->validateDie( __( 'Access denied', 'page_builder' ) )->wpAny( array(
					'edit_post',
					(int) vc_request_param( 'post_id' ),
				) )->validateDie( __( 'Access denied', 'page_builder' ) )->check( 'vc_user_access_check_shortcode_edit', $tag )->validateDie( __( 'Access denied', 'page_builder' ) );
