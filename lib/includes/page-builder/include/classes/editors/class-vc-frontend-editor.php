...
class Vc_Frontend_Editor implements Vc_Editor_Interface {
	...
	protected $settings = array(
		...
		'plugin_path' => 'page_builder/inline',
	);
	/**
	 * @var string
	 */
	...
	function setEditorTitle() {
		return sprintf( __( 'Edit %s with WPBakery Page Builder', 'page_builder' ), $this->post_type->labels->singular_name );
	}
	...
	function setEmptyTitlePlaceholder( $title ) {
		return ! is_string( $title ) || strlen( $title ) === 0 ? __( '(no title)', 'page_builder' ) : $title;
	}
	...
	function renderEditButton( $link ) {
		if ( $this->showButton( get_the_ID() ) ) {
			return $link . ' <a href="' . self::getInlineUrl() . '" id="vc_load-inline-editor" class="vc_inline-link">' . __( 'Edit with WPBakery Page Builder', 'page_builder' ) . '</a>';
		}
	...
	function renderRowAction( $actions ) {
		...
		if ( $this->showButton( $post->ID ) ) {
			$actions['edit_vc'] = '<a
		href="' . $this->getInlineUrl( '', $post->ID ) . '">' . __( 'Edit with WPBakery Page Builder', 'page_builder' ) . '</a>';
		}
		...
		if ( is_singular() ) {
			if ( $this->showButton( get_the_ID() ) ) {
				$wp_admin_bar->add_menu( array(
					...
					'title' => __( 'Edit with WPBakery Page Builder', 'page_builder' ),
	...
	function loadIFrameJsCss() {
		...
		wp_enqueue_style( 'vc_inline_css', vc_asset_url( 'css/page_builder_frontend_editor_iframe.min.css' ), array(), WPB_VC_VERSION );
	...
	public function registerCss() {
		...
		wp_register_style( 'vc_inline_css', vc_asset_url( 'css/page_builder_frontend_editor.min.css' ), array(), WPB_VC_VERSION );
