...
class Vc_Navbar_Frontend extends Vc_Navbar {
	...
	public function getControlGuidesSwitch() {
		return '<li class="vc_pull-right">'
		       ...
		       . ' title="' . __( "Toggle editor's guides", 'page_builder' ) . '">'
		       . __( 'Guides ON', 'page_builder' )
		       . '</button>'
		       . '</li>';
	}
	...
	public function getControlScreenSize() {
		...
		if ( '1' !== $disable_responsive ) {
			$screen_sizes = array(
				array(
					'title' => __( 'Desktop', 'page_builder' ),
					'size' => '100%',
					'key' => 'default',
					'active' => true,
				),
				array(
					'title' => __( 'Tablet landscape mode', 'page_builder' ),
					'size' => '1024px',
					'key' => 'landscape-tablets',
				),
				array(
					'title' => __( 'Tablet portrait mode', 'page_builder' ),
					'size' => '768px',
					'key' => 'portrait-tablets',
				),
				array(
					'title' => __( 'Smartphone landscape mode', 'page_builder' ),
					'size' => '480px',
					'key' => 'landscape-smartphones',
				),
				array(
					'title' => __( 'Smartphone portrait mode', 'page_builder' ),
					'size' => '320px',
			...
			$output = '<li class="vc_pull-right">'
			          ...
			          . ' title="' . __( 'Responsive preview', 'page_builder' ) . '"><i class="vc-composer-icon vc_current-layout-icon vc-c-icon-layout_default"'
	...
	public function getControlSaveUpdate() {
		...
		<li class="vc_show-mobile vc_pull-right">
			<button data-url="<?php esc_attr_e( get_edit_post_link( $post->ID ) . '&wpb_vc_js_status=true' ) ?>"
			        ...
			        title="<?php _e( 'Cancel all changes and return to WP dashboard', 'page_builder' ) ?>"><?php echo vc_user_access()
					->part( 'backend_editor' )
					->can()
					->get() ? __( 'Backend Editor', 'page_builder' ) : __( 'Edit', 'page_builder' ); ?></button>
			<?php if ( ! in_array( $post->post_status, array(
				...
				<?php if ( 'draft' === $post->post_status ) : ?>
					<button type="button" class="vc_btn vc_btn-default vc_btn-sm vc_navbar-btn vc_btn-save-draft"
					        ...
					        title="<?php esc_attr_e( 'Save Draft', 'page_builder' ) ?>"><?php _e( 'Save Draft', 'page_builder' ) ?></button>
				<?php elseif ( 'pending' === $post->post_status && $can_publish ) : ?>
					<button type="button" class="vc_btn vc_btn-primary vc_btn-sm vc_navbar-btn vc_btn-save"
					        id="vc_button-save-as-pending"
					        title="<?php esc_attr_e( 'Save as Pending', 'page_builder' ) ?>"><?php _e( 'Save as Pending', 'page_builder' ) ?></button>
				<?php endif ?>
				<?php if ( $can_publish ) : ?>
					<button type="button" class="vc_btn vc_btn-primary vc_btn-sm vc_navbar-btn vc_btn-save"
					        id="vc_button-update" title="<?php esc_attr_e( 'Publish', 'page_builder' ) ?>"
					        data-change-status="publish"><?php _e( 'Publish', 'page_builder' ) ?></button>
				<?php else : ?>
					<button type="button" class="vc_btn vc_btn-primary vc_btn-sm vc_navbar-btn vc_btn-save"
					        id="vc_button-update" title="<?php esc_attr_e( 'Submit for Review', 'page_builder' ) ?>"
					        data-change-status="pending"><?php _e( 'Submit for Review', 'page_builder' ) ?></button>
				<?php endif ?>
			<?php else : ?>
				<button type="button" class="vc_btn vc_btn-primary vc_btn-sm vc_navbar-btn vc_btn-save"
				        id="vc_button-update"
				        title="<?php esc_attr_e( 'Update', 'page_builder' ) ?>"><?php _e( 'Update', 'page_builder' ) ?></button>
			<?php endif ?>
		</li>
		<?php
	...
	public function getControlViewPost() {
		return '<li class="vc_pull-right">'
		       ...
		       . ' title="' . esc_attr__( 'Exit WPBakery Page Builder edit mode', 'page_builder' ) . '"><i class="vc-composer-icon vc-c-icon-close"></i></a>'
		       . '</li>';
	}
}
