...
while ( $my_query->have_posts() ) {
	...
	if ( 'link_no' !== $link ) {
		if ( 'link_post' === $link ) {
			$link_image_start = '<a class="link_image" href="' . get_permalink( $post_id ) . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'page_builder' ), the_title_attribute( 'echo=0' ) ) . '">';
...
if ( $teasers ) {
	...
	$teasers = __( 'Nothing found.', 'page_builder' );
}
