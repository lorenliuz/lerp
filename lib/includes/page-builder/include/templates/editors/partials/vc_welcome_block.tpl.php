...
<?php if ( vc_user_access()
	...
	<div id="vc_no-content-helper" class="vc_welcome vc_ui-font-open-sans">
		...
		<div class="vc_welcome-header vc_welcome-visible-e">
			<?php _e( 'You have blank page <br> Start adding content or templates', 'page_builder' ) ?>
		</div>
		<div class="vc_ui-btn-group vc_welcome-visible-e">
			...
			<?php if ( vc_user_access()
				...
				<a id="vc_no-content-add-text-block"
				   ...
				   title="<?php _e( 'Add text block', 'page_builder' ) ?>">
					<i class="vc-composer-icon vc-c-icon-text-block"></i>
					<span><?php _e( 'Add Text Block', 'page_builder' ) ?></span>
				</a>
			<?php endif ?>
			<?php if ( $templates_total_count > 0 && vc_user_access()
				...
				<a id="vc_templates-more-layouts"
				   class="vc_general vc_ui-button vc_ui-button-shape-rounded vc_ui-button-info" href="#">
					...
					<span><?php echo __( 'Add template', 'page_builder' ); ?></span>
				</a>
			<?php endif ?>
		</div>
		<?php
		if ( vc_user_access()
			...
			<div class="vc_welcome-visible-ne">
				<a id="vc_not-empty-add-element" class="vc_add-element-not-empty-button"
				   title="<?php _e( 'Add Element', 'page_builder' ) ?>" data-vc-element="add-element-action">
		...
		<p class="vc_ui-help-block vc_welcome-visible-e">
			<?php
			$targetLink = '<a href="http://kb.wpbakery.com" target="_blank">' . __( 'knowledge base', 'page_builder' ) . '</a>';
			$targetText = __( 'Don\'t know where to start? Visit our %s.', 'page_builder' );
