...
<div class="vc_ui-font-open-sans vc_ui-panel-window vc_media-xs vc_ui-panel"
	...
	<div class="vc_ui-panel-window-inner">
		<?php vc_include_template( 'editors/popups/vc_ui-header.tpl.php', array(
			'title' => __( 'Add Element', 'page_builder' ),
				...
				<div class="wpb-elements-list vc_filter-all" data-vc-ui-filter="*"
					...
					<ul class="wpb-content-layouts-container">
						<li class="vc_add-element-deprecated-warning">
							<div class="wpb_element_wrapper">
								<?php echo vc_message_warning( __( 'Elements within this list are deprecated and are no longer supported in newer versions of WPBakery Page Builder.', 'page_builder' ) ) ?>
							</div>
						</li>
						...
						<?php if ( $box->isShowEmptyMessage() && true !== $box->getPartState() ) : ?>
							<li class="vc_add-element-access-warning">
								<div class="wpb_element_wrapper">
									<?php echo vc_message_warning( __( 'Your user role have restricted access to content elements. If required, contact your site administrator to change WPBakery Page Builder Role Manager settings for your user role.', 'page_builder' ) ) ?>
								</div>
							</li>
						<?php endif; ?>
					</ul>
					...
					<?php if ( vc_user_access()->part( 'presets' )->checkStateAny( true, null )->get() ) : ?>
						<div class="vc_align_center">
							<span class="vc_general vc_ui-button vc_ui-button-action vc_ui-button-shape-rounded vc_ui-button-fw" data-vc-manage-elements style="display:none;"><?php _e( 'Manage elements', 'page_builder' ); ?></span>
						</div>
					<?php endif; ?>
				</div>
			</div>
