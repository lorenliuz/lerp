...
<?php if ( 'updater' === $tab && $dev_environment && ! vc_license()->isActivated() ) : ?>
	...
	<div class="updated vc_updater-result-message">
		<p>
			<strong>
				<?php _e( 'It is optional to activate license on localhost development environment. You can still activate license on localhost to receive plugin updates and get access to template library.', 'page_builder' ) ?>
			</strong>
		</p>
	</div>
<?php endif ?>
...
>
	...
	<?php if ( 'general' === $tab && vc_pointers_is_dismissed() ) : ?>
		<table class="form-table">
			<tr>
				<th scope="row"><?php _e( 'Guide tours', 'page_builder' ) ?></th>
				<td>
					<a href="#" class="button vc_pointers-reset-button"
						id="vc_settings-vc-pointers-reset"
						data-vc-done-txt="<?php _e( 'Done', 'page_builder' ) ?>"><?php _e( 'Reset', 'page_builder' ) ?></a>

					<p
						class="description indicator-hint"><?php _e( 'Guide tours are shown in VC editors to help you to start working with editors. You can see them again by clicking button above.', 'page_builder' ) ?></p>
				</td>
			</tr>
		</table>
	<?php endif ?>
	...
	<?php if ( 'updater' !== $tab ) : ?>
		<?php submit_button( __( 'Save Changes', 'page_builder' ), 'primary', 'submit_btn', true, $submit_button_attributes ); ?>
	<?php endif ?>
	...
	<?php if ( 'color' === $tab ) : ?>
		<a href="#" class="button vc_restore-button" id="vc_settings-color-restore-default">
			<?php echo __( 'Restore Default', 'page_builder' ) ?>
		</a>
	<?php endif ?>
	...
	<?php if ( 'updater' === $tab ) : ?>
		...
		<div class="vc_settings-activation-deactivation">
			<?php if ( vc_license()->isActivated() ) : ?>
				<p>
					<?php echo __( 'You have activated WPBakery Page Builder version which allows you to access all the customer benefits. Thank you for choosing WPBakery Page Builder as your page builder. If you do not wish to use WPBakery Page Builder on this WordPress site you can deactivate your license below.', 'page_builder' ) ?>
				</p>
				...
				<p>
					<button
						...
						<?php echo __( 'Deactivate WPBakery Page Builder', 'page_builder' ) ?>
					</button>
			...
			<?php else : ?>
				...
				<p>
					<button
						...
						type="button"
						id="vc_settings-updater-button">
						<?php echo __( 'Activate WPBakery Page Builder', 'page_builder' ) ?>
					</button>
				...
				<p class="description">
					<?php echo sprintf( __( 'Don\'t have direct license yet? <a href="%s" target="_blank">Purchase WPBakery Page Builder license</a>.', 'page_builder' ), esc_url( 'http://bit.ly/vcomposer' ) ) ?>
				</p>
