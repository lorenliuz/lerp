...
<div class="vc_column-offset" data-column-offset="true">
	<?php if ( '1' === vc_settings()->get( 'not_responsive_css' ) ) : ?>
		<div class="wpb_alert wpb_content_element vc_alert_rounded wpb_alert-warning">
			<div class="messagebox_text">
				<p><?php printf( __( 'Responsive design settings are currently disabled. You can enable them in WPBakery Page Builder <a href="%s">settings page</a> by unchecking "Disable responsive content elements".', 'page_builder' ), admin_url( 'admin.php?page=vc-general' ) ) ?></p>
			</div>
		</div>
	<?php endif ?>
	...
	<table class="vc_table vc_column-offset-table">
		<tr>
			<th>
				<?php _e( 'Device', 'page_builder' ) ?>
			</th>
			<th>
				<?php _e( 'Offset', 'page_builder' ) ?>
			</th>
			<th>
				<?php _e( 'Width', 'page_builder' ) ?>
			</th>
			<th>
				<?php _e( 'Hide on device?', 'page_builder' ) ?>
			</th>
		</tr>
...
<script type="text/javascript">
	window.VcI8nColumnOffsetParam = <?php echo json_encode( array(
		'inherit' => __( 'Inherit: ', 'page_builder' ),
		'inherit_default' => __( 'Inherit from default', 'page_builder' ),
	) ) ?>;
</script>
