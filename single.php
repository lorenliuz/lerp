...
if ( isset($metabox_data['_lerp_active_sidebar'][0]) && $metabox_data['_lerp_active_sidebar'][0] !== '' ) {
    if ( $metabox_data['_lerp_active_sidebar'][0] !== 'off' ) {
        ...
        $sidebar_sticky = (isset($metabox_data['_lerp_sidebar_sticky'][0]) && $metabox_data['_lerp_sidebar_sticky'][0] === 'on') ? ' sticky-element sticky-sidebar' : '';
...
while ( have_posts() ):
    ...
    if ( $show_breadcrumb && !is_front_page() && !is_home() ) {
        ...
        echo lerp_get_row_template($breadcrumb_title . $content_breadcrumb, '', ($page_custom_width !== '' ? ' limit-width' : $limit_content_width), $style, ' row-breadcrumb row-breadcrumb-' . $style . $breadcrumb_align, 'half', true, 'half');
    }
    ...
    if ( $show_title ) {
        ...
        $title_content .= lerp_post_info() . '</div>';
    ...
    $page_show_tags = (isset($metabox_data['_lerp_specific_tags'][0])) ? $metabox_data['_lerp_specific_tags'][0] : '';
    if ( $page_show_tags === '' ) {
        $generic_show_tags = ot_get_option('_lerp_' . $post_type . '_tags');
        $show_tags = ($generic_show_tags === 'off') ? false : true;
        if ( $show_tags ) $show_tags_align = ot_get_option('_lerp_' . $post_type . '_tags_align');
    } else {
        $show_tags = ($page_show_tags === 'off') ? false : true;
        if ( $show_tags ) $show_tags_align = (isset($metabox_data['_lerp_specific_tags_align'][0])) ? $metabox_data['_lerp_specific_tags_align'][0] : '';
    }
    ...
    if ( $show_tags ) {
        ...
        if ( $tag_ids ) {
            ...
            if ( !$with_builder ) $the_content .= $tag_cloud;
                ...
                $the_content .= lerp_get_row_template($tag_cloud, $limit_width, $limit_content_width, $style, '', false, true, 'double', $page_custom_width);
            }
        }
    }
    ...
    if ( shortcode_exists('jetpack-related-posts') ) {
        ...
        if ( $related_content !== '' ) {
            if ( !$with_builder ) $the_content .= $related_content;
                ...
                $the_content .= lerp_get_row_template($related_content, $limit_width, $limit_content_width, $style, '', false, true, 'double', $page_custom_width);
            }
        }
    }
    ...
    foreach ( $page_content_blocks_after as $order => $pre ) {
        ...
        $page_content_block_after = (isset($metabox_data['_lerp_specific_content_block_after' . $pre][0])) ? $metabox_data['_lerp_specific_content_block_after' . $pre][0] : '';
        if ( $page_content_block_after === '' ) {
            $generic_content_block_after = ot_get_option('_lerp_' . $post_type . '_content_block_after' . $pre);
        ...
        if ( $content_block_after !== false ) {
            ...
            else $content_after_body_build = '<div class="post-after row-container">' . lerp_get_row_template($content_after_body_build, $limit_width, $limit_content_width, $style, '', false, true, 'double', $page_custom_width) . '</div>';
    ...
    if ( $layout === 'sidebar_right' || $layout === 'sidebar_left' ) {
        ...
        if ( $with_builder ) {
            ...
            $lerp_get_row_template = lerp_get_row_template($the_content, $limit_width, $limit_content_width, $style, '', 'double', true, 'double');
            if ( $lerp_get_row_template != '' )
                $the_content = '<div class="post-content"' . $page_custom_width . '>' . $lerp_get_row_template . '</div>';
        }
        $the_content .= $content_after_body;
        $lerp_get_row_template = lerp_get_row_template($footer_content, $limit_width, $limit_content_width, $style, '', false, true, 'double', $page_custom_width);
        if ( $lerp_get_row_template != '' )
            $the_content .= '<div class="post-footer post-footer-' . $style . ' row-container">' . $lerp_get_row_template . '</div>';
    }
