(function($) {
		...
		if (window.wc_add_to_cart_params != undefined) {
			$.post({
				...
				success: function(data, textStatus, XMLHttpRequest) {
					$('.lerp-cart-dropdown').html(data.cart);
					if (data != '') {
						if ($('.lerp-cart .badge, .mobile-shopping-cart .badge').length) {
							if (data.articles > 0) {
								$('.lerp-cart .badge, .mobile-shopping-cart .badge').html(data.articles);
								$('.lerp-cart .badge, .mobile-shopping-cart .badge').show();
							} else {
								$('.lerp-cart .badge, .mobile-shopping-cart .badge').hide();
							}
						} else $('.lerp-cart .cart-icon-container').append('<span class="badge">' + data.articles + '</span>'); //$('.lerp-cart .badge').html(data.articles);
					}
				}
			});
		}
	...
	function remove_from_cart() {
		$(document).on('click', '.lerp-cart-dropdown a.remove', function(e){
	...
	function change_images(event, variation) {
		if (variation.image_src !== '') {
			...
			if (image_variable.hasClass('async-done')) {
				image_variable.attr('data-path', variation.lerp_image_path);
				image_variable.attr('data-guid', variation.lerp_image_guid);
