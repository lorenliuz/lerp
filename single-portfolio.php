...
else
{
	...
	{
		$metabox_data['_lerp_header_type'] = array(
			$page_header_type
		);
...
/** Get layout info **/
if (isset($metabox_data['_lerp_portfolio_active'][0]) && $metabox_data['_lerp_portfolio_active'][0] !== '')
{
	/** Page specific info **/
	if ($metabox_data['_lerp_portfolio_active'][0] !== 'off')
	{
		$layout = (isset($metabox_data['_lerp_portfolio_position'][0])) ? $metabox_data['_lerp_portfolio_position'][0] : '';
		$sidebar_size = (isset($metabox_data['_lerp_portfolio_sidebar_size'][0])) ? $metabox_data['_lerp_portfolio_sidebar_size'][0] : 4;
		$sidebar_sticky = (isset($metabox_data['_lerp_portfolio_sidebar_sticky'][0]) && $metabox_data['_lerp_portfolio_sidebar_sticky'][0] === 'on') ? ' sticky-element sticky-sidebar' : '';
		$sidebar_fill = (isset($metabox_data['_lerp_portfolio_sidebar_fill'][0])) ? $metabox_data['_lerp_portfolio_sidebar_fill'][0] : '';
		$portfolio_style = (isset($metabox_data['_lerp_portfolio_style'][0])) ? $metabox_data['_lerp_portfolio_style'][0] : $style;
		$portfolio_bg_color = (isset($metabox_data['_lerp_portfolio_bgcolor'][0]) && $metabox_data['_lerp_portfolio_bgcolor'][0] !== '') ? ' style-' . $metabox_data['_lerp_portfolio_bgcolor'][0] . '-bg' : '';
	}
}
else
{
	/** Page generic info **/
	$layout = ot_get_option('_lerp_' . $post_type . '_position');
	if ($layout !== 'off')
	{
		$portfolio_style = ot_get_option('_lerp_' . $post_type . '_style');
		$sidebar_size = ot_get_option('_lerp_' . $post_type . '_size');
		...
		$portfolio_bg_color = ot_get_option('_lerp_' . $post_type . '_bgcolor');
...
if (!empty($portfolio_details))
{
	...
	{
		$portfolio_detail = (isset($metabox_data[$value['_lerp_portfolio_detail_unique_id']][0])) ? $metabox_data[$value['_lerp_portfolio_detail_unique_id']][0] : '';
		if ($portfolio_detail !== '')
		{
...
while (have_posts()):
	...
	if ($show_breadcrumb && !is_front_page() && !is_home())
	{
		...
		if (isset($metabox_data['_lerp_specific_navigation_index'][0]) && $metabox_data['_lerp_specific_navigation_index'][0] !== '') {
			$navigation_index = $metabox_data['_lerp_specific_navigation_index'][0];
		} else {
			$navigation_index = ot_get_option('_lerp_' . $post_type . '_navigation_index');
		}
		$content_breadcrumb = lerp_breadcrumbs($navigation_index);
	...
	$text_content = lerp_the_content(get_the_excerpt());
	...
	$generic_show_share = ot_get_option('_lerp_' . $post_type . '_share');
	$page_show_share = (isset($metabox_data['_lerp_specific_share'][0])) ? $metabox_data['_lerp_specific_share'][0] : '';
	if ($page_show_share === '')
	{
	...
	if ($show_share)
		$footer_content = '<div class="post-share">
	          						<div class="detail-container">
													<span class="detail-label">' . esc_html__('Share', 'lerp') . '</span>
	...
	if ((comments_open() || '0' != get_comments_number()) && $show_comments === 'on')
	{
		...
		$comment_content = lerp_get_row_template($comment_content, '', $limit_content_width, $style, ' portfolio-comments portfolio-comments-' . $style, false, true, 'double');
	}
	...
	if ($layout === 'sidebar_right' || $layout === 'sidebar_left')
	{
		...
		else
		{
			...
			{
				$the_content = lerp_get_row_template($the_content, '', '', $style, ' limit-width', ($media_content !== '' ? true : 'double') , false, 'double');
				$the_content = '<div class="post-content">' . $the_content . '</div>';
				if ($media_content !== '') $the_content = lerp_get_row_template($media_content, '', $limit_content_width, $style, '', false, false, false) . $the_content;
			}
			else
			{
				if ($media_content !== '') $media_content = lerp_get_row_template($media_content, '', $limit_content_width, $style, '', false, false, 'double');
		...
		if ($footer_content !== '') {
			if ($limit_content_width === '') $footer_content = lerp_get_row_template($footer_content, $limit_width, $limit_content_width, $style, '', false, false, '');
	...
	if ($layout === 'portfolio_top' || $layout === 'portfolio_bottom')
	{
		...
		else
		{
			if ($the_content !== '' && !empty($the_content))
			{
				$the_content = lerp_get_row_template($the_content, '', '', $style, ' limit-width', ($layout === 'portfolio_top' ? false : 'double') , false, ($layout === 'portfolio_top' ? 'double' : false));
				$the_content = '<div class="post-content">' . $the_content . '</div>';
			}
			if ($media_content !== '') $media_content = lerp_get_row_template($media_content, '', $limit_content_width, $style, '', ($layout === 'portfolio_top' && $portfolio_bg_color === '' ? false : 'double') , true, ($layout === 'portfolio_bottom' && $portfolio_bg_color === '' ? false : 'double'));
			$the_content = $media_content . $the_content;
		}
	...
	if ($layout === '')
	{
		...
		{
			if ($title_content !== '' || $the_content !== '') $the_content = lerp_get_row_template($title_content . $the_content, '', '', $style, ' limit-width', ($media_content === '' ? 'double' : false), false, 'double');
			if ($media_content !== '') $media_content = lerp_get_row_template($media_content, '', $limit_content_width, $style, '', 'double', true, 'double');
			$the_content = $media_content . $the_content;
		}
		$the_content = '<div class="post-content">' . $the_content . '</div>';
	}
	...
	if ($navigation_option !== 'off')
	{
		...
		if ($navigation_index !== '')
		{
			$navigation_index_label = ot_get_option('_lerp_' . $post_type . '_navigation_index_label');
		...
		$navigation_nextprev_title = ot_get_option('_lerp_' . $post_type . '_navigation_nextprev_title');
		$navigation = lerp_post_navigation($navigation_index_btn, $navigation_nextprev_title, $navigation_index, $generic_index);
	...
	echo '<div class="page-body' . $bg_color . '">
					<div class="portfolio-wrapper"' . $page_custom_width . '>
						<div class="portfolio-body">' . lerp_remove_wpautop($the_content) . '</div>' .
						 lerp_remove_wpautop($content_after_body) . $comment_content .
					'</div>
				</div>'
				. $navigation_content;
endwhile;
