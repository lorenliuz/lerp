...
class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu
{
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        if ( !empty($item->_invalid) ) {
            ...
            $title = sprintf(esc_html__('%s (Invalid)', 'lerp'), $item->title);
        } elseif ( isset($item->post_status) && 'draft' == $item->post_status ) {
            $classes[] = 'pending';
            /* translators: %s: title of menu item in draft status */
            $title = sprintf(esc_html__('%s (Pending)', 'lerp'), $item->title);
        }

        $title = (!isset($item->label) || '' == $item->label) ? $title : $item->label;

        if ( $item->megamenu ) $parent_mega = $item->ID;
        if ( $parent_mega == $item->menu_item_parent ) $sublabel = esc_html__('MEGAMENU TITLE', 'lerp');
        else $sublabel = $item->megamenu ? esc_html__('MEGAMENU', 'lerp') : esc_html__('sub item', 'lerp');
        ...
        <dl class="menu-item-bar">
            <dt class="menu-item-handle">
                <span class="item-title"><span class="menu-item-title"><?php echo esc_html($title); ?></span> <span
                            class="is-submenu" <?php echo wp_kses_post($submenu_text); ?>><?php echo wp_kses_post($sublabel); ?></span></span>
                <span class="item-controls">
						...
						<span class="item-order hide-if-js">
                            ...
                            ?>" class="item-move-up"><abbr
                                        title="<?php esc_attr_e('Move up', 'lerp'); ?>">&#8593;</abbr></a>
							|
							<a href="<?php
                            echo wp_nonce_url(
                                add_query_arg(
                            ...
                            ?>" class="item-move-down"><abbr
                                        title="<?php esc_attr_e('Move down', 'lerp'); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>"
                           title="<?php esc_attr_e('Edit Menu Item', 'lerp'); ?>" href="<?php
                        echo (isset($_GET['edit-menu-item']) && $item_id == $_GET['edit-menu-item']) ? admin_url('nav-menus.php') : add_query_arg('edit-menu-item', $item_id, remove_query_arg($removed_args, admin_url('nav-menus.php#menu-item-settings-' . $item_id)));
                        ?>"><span class="screen-reader-text"><?php esc_html_e('Edit Menu Item', 'lerp'); ?></span></a>
					</span>
            </dt>
        </dl>
        ...
        <div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Navigation Label', 'lerp'); ?><br/>
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Title Attribute', 'lerp'); ?><br/>
            ...
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                    ...
                    <?php esc_html_e('Open link in a new window/tab', 'lerp'); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('CSS Classes (optional)', 'lerp'); ?><br/>
            ...
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Link Relationship (XFN)', 'lerp'); ?><br/>
            ...
            <p class="field-custom description description-wide">
                <label for="edit-menu-item-icon-<?php echo esc_attr($item_id); ?>">
                    <input type="text" class="menu_icon_container"
            ...
            <p class="field-custom description description-wide">
                <label for="edit-menu-item-button-<?php echo esc_attr($item_id); ?>">
                    <input type="checkbox" id="edit-menu-item-button-<?php echo esc_attr($item_id); ?>" value="button"
                           name="menu-item-button[<?php echo esc_attr($item_id); ?>]"<?php checked($item->button, 'button'); ?> />
                    <?php esc_html_e('Button style', 'lerp'); ?>
                </label>
            </p>

            <p class="field-move hide-if-no-js description description-wide">
                <label>
                    <span><?php esc_html_e('Move', 'lerp'); ?></span>
                    <a href="#" class="menus-move-up"><?php esc_html_e('Up one', 'lerp'); ?></a>
                    <a href="#" class="menus-move-down"><?php esc_html_e('Down one', 'lerp'); ?></a>
                    <a href="#" class="menus-move-left"></a>
                    <a href="#" class="menus-move-right"></a>
                    <a href="#" class="menus-move-top"><?php esc_html_e('To the top', 'lerp'); ?></a>
                </label>
            </p>
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if ( 'custom' != $item->type && $original_title !== false ) : ?>
                    <p class="link-to-original">
                        <?php printf(wp_kses(__('Original: %s', 'lerp'), array('a' => array('href' => array()))), '<a href="' . esc_attr($item->url) . '">' . esc_html($original_title) . '</a>'); ?>
                    </p>
                <?php endif; ?>
                ...
                echo wp_nonce_url(
                    ...
                    'delete-menu_item_' . $item_id
                ); ?>"><?php esc_html_e('Remove', 'lerp'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a
                        ...
                        ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'lerp'); ?></a>
            </div>
