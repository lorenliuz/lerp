...
function lerp_addScripts()
{
    ...
    <script>
        ...
        arrayRegex[0] = {
            domain: '((http|https):\/\/)?(www\.)?(youtube\.com|youtu\.be)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/youtube'
        };
        arrayRegex[1] = {
            domain: '((http|https):\/\/)?(www\.)?(blip\.tv)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/blip'
        };
        arrayRegex[2] = {
            domain: '((http|https):\/\/)?(www\.)?(vimeo\.com)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/vimeo'
        };
        arrayRegex[3] = {
            domain: '((http|https):\/\/)?(www\.)?(dailymotion\.com)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/dailymotion'
        };
        arrayRegex[4] = {
            domain: '((http|https):\/\/)?(www\.)?(qik\.com)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/qik'
        };
        arrayRegex[5] = {
            domain: '((http|https):\/\/)?(www\.)?(flickr\.com)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/flickr'
        };
        arrayRegex[6] = {
            domain: '((http|https):\/\/)?(www\.)?(hulu\.com)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/hulu'
        };
        arrayRegex[7] = {
            domain: '((http|https):\/\/)?(www\.)?(viddler\.com)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/viddler'
        };
        arrayRegex[8] = {
            domain: '((http|https):\/\/)?(www\.)?(slideshare\.net)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/slideshare'
        };
        arrayRegex[9] = {
            domain: '((http|https):\/\/)?(www\.)?(wordpress\.tv)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/wordpress'
        };
        arrayRegex[10] = {
            domain: '((http|https):\/\/)?(www\.)?(twitter\.com)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/twitter'
        };
        arrayRegex[11] = {
            domain: '((http|https):\/\/)?(www\.)?(scribd\.com)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/scribd'
        };
        arrayRegex[12] = {
            domain: '((http|https):\/\/)?(www\.)?(photobucket\.com)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/photobucket'
        };
        arrayRegex[13] = {
            domain: '((http|https):\/\/)?(www\.)?(soundcloud\.com)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/soundcloud'
        };
        arrayRegex[14] = {
            domain: '((http|https):\/\/)?(www\.)?(instagr\.am|instagram\.com)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/instagram'
        };
        arrayRegex[15] = {
            domain: '((http|https):\/\/)?(www\.)?(flic\.kr)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/flickr'
        };
        arrayRegex[16] = {domain: '((http|https):\/\/.+?\.jpg|jpeg|png|gif|tiff/gi)/?', type: 'image/url'};
        arrayRegex[17] = {
            domain: '((http|https):\/\/)?(www\.)?(spotify\.com)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/spotify'
        };
        arrayRegex[18] = {
            domain: '((http|https):\/\/)?(www\.)?(facebook\.com)(\/)?([a-zA-Z0-9\-\.]+)\/?',
            type: 'oembed/facebook'
        };
...
function lerp_addOtherMedia()
{
    ...
    <div class="wrap">
        ...
        <h2><?php esc_html_e("Add oEmbed, external IMG, SVG code, HTML or Shortcode", 'lerp'); ?> <a
                    href="upload.php?page=add-other" class="add-new-h2"><?php esc_html_e('Add New', 'lerp'); ?></a></h2>
        <div id="poststuff">
            <?php
            if ( isset($_REQUEST['postid']) ) { ?>
                <div id="message" class="updated below-h2"><p><?php esc_html_e('Item added to library.', 'lerp'); ?></p>
                </div>
            <?php } ?>
            <?php
                        ...
                        <div class="omedia-frame-content">
                            <div id="content">
                                <div id="titlediv" class="controls">
                                    <div id="titlewrap">
                                        <input placeholder="<?php esc_html_e('Enter title here*', 'lerp'); ?>"
                                               type="text" name="mle-title" size="30"
                                               value="<?php if ( isset($post) ) echo esc_attr($post->post_title); ?>"
                                        ...
                                        <div class="inside">
                                            <div id="edit-slug-box" class="hide-if-no-js">
                                                ...
                                                <span id="sample-permalink"
                                                      tabindex="-1"><?php echo get_site_url() . '/?attachment_id=' . $_REQUEST['postid']; ?></span>
                                                <span id="view-post-btn"><a
                                                            href="<?php echo get_site_url() . '/?attachment_id=' . $_REQUEST['postid']; ?>"
                                                            class="button button-small"><?php esc_html_e('View Post', 'lerp'); ?></a></span>
                                            </div>
                                        </div>
                                    <?php } ?>
                                </div>
                                <div class="media-embed controls section">
                                    ...
                                    <textarea id="linkUrl" rows="10" name="mle-code"
                                              class="embed"><?php if ( isset($post) ) wp_kses_post($get_code); ?></textarea>
                                    <input type="hidden" name="mle-mime" id="mle-mime"
                                           value="<?php if ( isset($post) ) echo esc_attr($post->post_mime_type); ?>">
                                    <div class="oembed"></div>
                                    <div class="oembed_code" style="display: none;"></div>
                                </div>
                                <div class="media-caption controls section">
                                    <p><strong><?php esc_html_e('Caption', 'lerp'); ?></strong></p>
                                    <input type="text" name="mle-caption" class="alignment" data-setting="caption"
                                           value="<?php if ( isset($post) ) echo esc_attr($post->post_excerpt); ?>">
                                </div>
                            ...
                            <input type="hidden" name="nonce"
                                   value="<?php echo wp_create_nonce("lerp-recordmedia-nonce"); ?>">
                            <?php
                            if ( isset($_REQUEST['postid']) ) { ?>
                                ...
                                <div id="major-publishing-actions">
                                    <div id="publishing-action">
                                        ...
                                        <input name="original_publish" type="hidden" id="original_publish"
                                               value="<?php esc_html_e('Publish', 'lerp'); ?>">
                                        <?php
                                        if ( !isset($_REQUEST['postid']) ) { ?>
                                            <input name="save" type="submit" class="button button-primary button-large"
                                                   id="publish"
                                                   data-url="<?php echo get_site_url() . '/wp-admin/upload.php?page=add-other'; ?>"
                                                   value="<?php esc_html_e('Save', 'lerp'); ?>">
                                        <?php } ?>
                                        <?php
                                        if ( isset($_REQUEST['postid']) ) { ?>
                                            <input name="save" type="submit" class="button button-primary button-large"
                                                   id="publish"
                                                   data-url="<?php echo get_site_url() . '/wp-admin/upload.php?page=add-other&postid=' . $_REQUEST['postid'] . '&updated=1'; ?>"
                                                   value="<?php esc_html_e('Update', 'lerp'); ?>">
                                        <?php } ?>
                                    </div>
                                    <div class="clear"></div>
                                </div>
