...
if ( ! function_exists( 'lerp_save_gallery_media' ) ) :
/**
 * Create/update a new media when the gallery cpt is published
 */
function lerp_save_gallery_media( $post_id, $post, $update ) {
	...
	remove_action( 'save_post_lerp_gallery', 'lerp_save_gallery_media', 10, 3 );
	...
	try {
		...
		$post_gallery_media_id = lerp_get_gallery_attached_media( $post_id );
		...
		if ( $update && $post_gallery_media_id ) {
			...
			$args = array(
				...
				'meta_input'     => array(
					'_lerp_gallery_cover'     => $thumb,
					'_lerp_is_oembed_gallery' => true,
				)
			);
		...
		// New post
			...
			update_post_meta( $post_id, '_lerp_gallery_media', $media_id );
		}

	} catch ( Exception $e ) {
		return new WP_Error( 'lerp_gallery_media_error', $e->getMessage() );
	}

	// Re-hook this function
	add_action( 'save_post_lerp_gallery', 'lerp_save_gallery_media', 10, 3 );
}
endif;
add_action( 'save_post_lerp_gallery', 'lerp_save_gallery_media', 10, 3 );

if ( ! function_exists( 'lerp_get_gallery_attached_media' ) ) :
/**
 ...
 */
function lerp_get_gallery_attached_media( $post_id ) {
	$post_gallery_media_id = get_post_meta( $post_id, '_lerp_gallery_media', true );
...
if ( ! function_exists( 'lerp_trash_gallery_media' ) ) :
/**
 * When a gallery cpt is trashed, delete the gallery media attachment permanently
 */
function lerp_trash_gallery_media( $post_id, $post ) {
	$post_gallery_media_id = lerp_get_gallery_attached_media( $post_id );
...
add_action( 'trash_lerp_gallery', 'lerp_trash_gallery_media', 10, 2 );

if ( ! function_exists( 'lerp_media_attachment_data' ) ) :
/**
 ...
 */
function lerp_media_attachment_data( $response, $attachment, $meta ) {
	if ( $response[ 'mime' ] == 'oembed/gallery' ) {
		// Check if the attachment has a gallery cover
		$gallery_cover_id     = get_post_meta( $attachment->ID, '_lerp_gallery_cover', true );
...
add_filter( 'wp_prepare_attachment_for_js', 'lerp_media_attachment_data', 10, 3 );

if ( ! function_exists( 'lerp_hide_gallery_attachments_grid_view' ) ) :
/**
 * Grid View
 *
 * Filter the media upload query to hide gallery attachements.
 * Gallery attachments will be visible only in the VC modules
 * and in the Lerp "Select medias" uploaders
 */
function lerp_hide_gallery_attachments_grid_view( $query ) {
	...
	$media_attachments_ids = lerp_get_gallery_attachment_ids();
...
add_filter( 'ajax_query_attachments_args', 'lerp_hide_gallery_attachments_grid_view' );

if ( ! function_exists( 'lerp_hide_gallery_attachments_list_view' ) ) :
/**
 * List View
 *
 ...
 * medias in List mode
 */
function lerp_hide_gallery_attachments_list_view( $query ){
    if ( ! is_admin() ) {
        return;
    }
...
add_action( 'pre_get_posts' , 'lerp_hide_gallery_attachments_list_view' );

if ( ! function_exists( 'lerp_get_gallery_attachment_ids' ) ) :
/**
 * Get the IDs of all the media (oembed/gallery) attachments
 *
 * @TODO: Maybe use a transient instead of WP Query?
 */
function lerp_get_gallery_attachment_ids() {
...
if ( ! function_exists( 'lerp_before_media_send_to_editor' ) ) :
/**
 * Before to send to editor the selected medias. We don't want our media
 * gallery attachments in the WP editor. So skip them.
 */
function lerp_before_media_send_to_editor( $html, $id, $attachment ) {
	if ( in_array( $id, lerp_get_gallery_attachment_ids() ) ) {
		return '';
	}
...
add_filter( 'media_send_to_editor', 'lerp_before_media_send_to_editor', 10, 3 );

if ( ! function_exists( 'lerp_get_album_item' ) ) :
/**
 * Print album item
 */
function lerp_get_album_item( $id ) {
	$album_item_attributes = lerp_get_media_info($id);
	...
	switch ($mime) {
		...
		case 'audio/mpeg':
		case 'video/mp4':
			$media_oembed = lerp_get_oembed($id, $album_item_attributes->guid, $album_item_attributes->post_mime_type, true, $album_item_attributes->post_excerpt, $album_item_attributes->post_content);
...
endif;//lerp_get_album_item

if ( ! function_exists( 'lerp_delete_translated_galleries' ) ) :
/**
 * Delete unwanted translations for the oembed/galleries created by WPML Media
 */
function lerp_delete_translated_galleries() {
	global $wpdb;
	...
	if ( count( $attachment_ids ) > 0 ) {
		foreach ( $attachment_ids as $row ) {
			$is_oembed_gallery = get_post_meta( $row[ 'ID' ], '_lerp_is_oembed_gallery', true );
...
add_action( 'wp_loaded', 'lerp_delete_translated_galleries' );
