...
(function ($) {
                ...
                if (data != '') {
                    try {
                        ...
                        if (parsedData.code == 'null') {
                            ...
                            $this.closest('.uploader-lerp-media').find('.spinner').removeClass('visible');
    ...
    $('.vc_welcome-header').html('Welcome to Lerp<br>Visual Composer Version');
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == 1) {
                    ...
                    window.location = 'admin.php?page=lerp-plugins';
                }
            }
        });
    });
...
(function ($) {
    ...
    $('#option-tree-settings-api.ajax-enabled').on('submit', function () {
        ...
        var _this = $(this).removeClass('lerp-ajax-error').removeClass('lerp-ajax-saved');
        var formData = _this.serialize();
        var $spinner = $('#option-tree-sub-header button.option-tree-save-button .lerp-ot-spinner'),
            timeOut;
        ...
        // Add loading class
        _this.addClass('lerp-ajax-loading');
        ...
        $.post('options.php', formData).fail(function (xhr, status, error) {
            ...
            _this.removeClass('lerp-ajax-loading').addClass('lerp-ajax-error').addClass('lerp-ajax-error-text');

            $('#theme-options-ajax-message').append('<span class="theme-options-ajax-message">' + message + '</span>').fadeIn(250);

            timeOut = setTimeout(function () {
                _this.removeClass('lerp-ajax-error');
                $('#theme-options-ajax-message').fadeOut(500, function () {
                    _this.removeClass('lerp-ajax-error-text');
                });
            }, 3000);
            ...
            if (res.indexOf('<body id="error-page">') !== -1) {
                ...
                _this.removeClass('lerp-ajax-loading').addClass('lerp-ajax-saved').addClass('lerp-ajax-saved-text');
            ...
            timeOut = setTimeout(function () {
                _this.removeClass('lerp-ajax-saved').removeClass('lerp-ajax-error');
                $('#theme-options-ajax-message').fadeOut(500, function () {
                    _this.removeClass('lerp-ajax-saved-text').removeClass('lerp-ajax-error-text');
                });
            }, 3000);
    ...
    function reloadDynamicData() {
        var hasPaletteChanged = hasDynamicListChanged('_lerp_custom_colors_list');
        var hasFontFamilyChanged = hasDynamicListChanged('_lerp_font_groups');
        var hasFontSizeChanged = hasDynamicListChanged('_lerp_heading_font_sizes');
        var hasLineHeightChanged = hasDynamicListChanged('_lerp_heading_font_heights');
        var hasLetterSpacingChanged = hasDynamicListChanged('_lerp_heading_font_spacings');
    ...
    function refreshColorPalettes() {
        $.ajax({
            url: ajaxurl,
            data: {
                action: 'lerp_get_colors'
            },
            success: function (response) {
                var dropdowns = $('.lerp-color-select');
                var optionsString = '';
                var listString = '';
    ...
    function refreshFontFamily() {
        $.ajax({
            url: ajaxurl,
            data: {
                action: 'lerp_get_font_family'
            },
            success: function (response) {
                var dropdowns = $('.lerp_font_family_dropdown');
                var optionsString = '';
    ...
    function refreshFontSize() {
        $.ajax({
            url: ajaxurl,
            data: {
                action: 'lerp_get_font_size'
            },
            success: function (response) {
                var dropdowns = $('.lerp_font_size_dropdown');
                var optionsString = '';
    ...
    function refreshLineHeight() {
        $.ajax({
            url: ajaxurl,
            data: {
                action: 'lerp_get_line_height'
            },
            success: function (response) {
                var dropdowns = $('.lerp_line_height_dropdown');
                var optionsString = '';
    ...
    function refreshLetterSpacing() {
        $.ajax({
            url: ajaxurl,
            data: {
                action: 'lerp_get_letter_spacing'
            },
            success: function (response) {
                var dropdowns = $('.lerp_letter_spacing_dropdown');
                var optionsString = '';
    ...
    function refreshCSS() {
        var cssURL = $('#lerp-custom-style-css').attr('href');
        cssURL = updateQueryStringParameter(cssURL, 'id', Math.random(0, 10000));
        $('#lerp-custom-style-css').attr('href', cssURL);
    }
...
(function ($) {
    ...
    UNCODE.dismissAdminNotices = function () {
        ...
        $('#lerp_vc_admin_notice').on('click', '.notice-dismiss', function () {

            $.ajax({
                url: ajaxurl,
                data: {
                    action: 'lerp_vc_admin_notice_dismiss'
                }
            });
    ...
    UNCODE.messagePosition = function () {
        ...
        var $form = $('#option-tree-settings-api'),
            ...
            $message = $('#lerp-ot-message');

        if (!$form.length || !$wrap.length || !$message.length)
            return;


        $form.addClass('lerp-ajax-saved-text');
        ...
        setTimeout(function () {
            $('#theme-options-ajax-message').fadeOut(500, function () {
                $form.removeClass('lerp-ajax-error-text');
            });
        }, 7000);
    ...
    UNCODE.managePageOptions = function () {
        $(".ot-numeric-slider-wrap").has('#_lerp_product_media_size').each(function () {
...
// 	$.fn.lerp_modal = function(action, content) {
// 		if (action === "open") {
// 			$('body').append('<div class="lerp-ui-modal"></div><div class="lerp-ui-overlay"></div>');

// 			var modal = $('.lerp-ui-modal');
// 			content.push('<button type="button" id="lerp-cancel-modal" class="lerp-ui-button lerp-ui-button--modal lerp-ui-button--cancel">' + SiteParameters.modal_buttons.cancel + '</button>');
// 			content.push('<button type="button" id="lerp-confirm-modal" class="lerp-ui-button lerp-ui-button--modal lerp-ui-button--confirm">' + SiteParameters.modal_buttons.confirm + '</button>');
// 			modal.append(content);
// 		}

// 		if (action === "close") {
// 			// Unbind handlers
// 			$(document).off('click', '#lerp-confirm-modal');

// 			$('.lerp-ui-modal, .lerp-ui-overlay').remove();
// 		}
// 	};
// })(jQuery);
