<?php
	...
	function lerp_add_tmpl_attachment() { ?>
				...
				<# } else { #>
					...
					<# if ( 'image' === data.type && ! data.uploading ) { #>
						...
						<# if ( data.can.save && data.sizes ) { #>
							<a class="edit-attachment" href="{{ data.editLink }}&amp;image-editor" target="_blank"><?php _e( 'Edit Image', 'lerp' ); ?></a>
						<# } #>
					<# } #>
					...
					<# if ( ! data.uploading && data.can.remove ) { #>
						...
						<# if ( 'trash' === data.status ) { #>
							<button type="button" class="button-link untrash-attachment"><?php _e( 'Untrash', 'lerp' ); ?></button>
						<# } else { #>
							<button type="button" class="button-link trash-attachment"><?php _ex( 'Trash', 'verb', 'lerp' ); ?></button>
						<# } #>
						<?php else: ?>
							<button type="button" class="button-link delete-attachment"><?php _e( 'Delete Permanently', 'lerp' ); ?></button>
						<?php endif; ?>
					<# } #>
				<# } #>
		...
		<# if ( 'oembed/gallery' === data.mime ) { #>
			<div><a class="edit-attachment" style="text-decoration:none;" href="{{ data.parentPostEditLink }}" target="_blank"><?php esc_html_e( 'View Gallery', 'lerp' ); ?></a></div>

			<# if ( ! data.uploading && data.can.remove ) { #>
				<?php if ( MEDIA_TRASH ): ?>
				<# if ( 'trash' === data.status ) { #>
					<button style="display: none;" type="button" class="button-link untrash-attachment"><?php _e( 'Untrash', 'lerp' ); ?></button>
				<# } else { #>
					<button style="display: none;" type="button" class="button-link trash-attachment"><?php _ex( 'Trash', 'verb', 'lerp' ); ?></button>
				<# } #>
				<?php else: ?>
					<button style="display: none;" type="button" class="button-link delete-attachment"><?php _e( 'Delete Permanently', 'lerp' ); ?></button>
				<?php endif; ?>
			<# } #>
		<# } #>
		...
		<script type="text/html" id="tmpl-attachment-details-two-column">
			...
			<div class="attachment-info">
				...
				<div class="details">
					<div class="filename"><strong><?php esc_html_e( 'File name:' , 'lerp') ; ?></strong> {{ data.filename }}</div>
					<div class="filename"><strong><?php esc_html_e( 'File type:' , 'lerp') ; ?></strong> {{ data.mime }}</div>
					<div class="uploaded"><strong><?php esc_html_e( 'Uploaded on:' , 'lerp') ; ?></strong> {{ data.dateFormatted }}</div>

					<div class="file-size"><strong><?php esc_html_e( 'File size:' , 'lerp') ; ?></strong> {{ data.filesizeHumanReadable }}</div>
					<# if ( 'image' === data.type && ! data.uploading ) { #>
						<# if ( data.width && data.height ) { #>
							<div class="dimensions"><strong><?php esc_html_e( 'Dimensions:' , 'lerp') ; ?></strong> {{ data.width }} &times; {{ data.height }}</div>
						<# } #>
					<# } #>

					<# if ( data.fileLength ) { #>
						<div class="file-length"><strong><?php esc_html_e( 'Length:' , 'lerp') ; ?></strong> {{ data.fileLength }}</div>
					<# } #>

					<# if ( 'audio' === data.type && data.meta.bitrate ) { #>
						<div class="bitrate">
							<strong><?php esc_html_e( 'Bitrate:' , 'lerp') ; ?></strong> {{ Math.round( data.meta.bitrate / 1000 ) }}kb/s
				...
				<div class="actions">
					<a class="view-attachment" href="{{ data.link }}"><?php esc_html_e( 'View attachment page' , 'lerp') ; ?></a> |
