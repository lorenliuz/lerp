!function ($) {
    ...
    window.itemIndex = function () {
        var $checkIfLoop = $('.wpb_el_type_loop .loop_field').length,
            $checkIfGallery = $('.lerp_gallery').length;
        if ($checkIfLoop == 0) {
        ...
        if ($checkIfLoop == 1) {
            ...
            $('#lerp_items_container, #lerp_matrix_items_container').sortable({
                axis: "y"
            });

            $('#lerp_items_container, #lerp_matrix_items_container').on("sortupdate", function (event, ui) {

                var $newIds = new Array(),
                    newBundle = {},
                    $ulThis = $(this),
                    $containerUl = $ulThis.parent(),
                    $bundleInput = $containerUl.find('.lerp_bundle_items'),
                    $tab_block = $ulThis.closest('.vc_edit-form-tab'),
                    $retrieveBundle;

                if ($ulThis.is('#lerp_matrix_items_container')) {
            ...
            $('select[name="post_matrix"]').on('change', function (event) {
                ...
                if ($(':selected', this).val() == 'matrix') {
                    load_item($('input.loop_field[name="loop"]').val(), $('#lerp_matrix_items_container'));
                }
            });

            $('.wpb_el_type_lerp_matrix_set_amount .matrix_amount').on('keyup', function () {
                $('.wpb_el_type_lerp_matrix_set_amount button').prop('disabled', false);
            });

            $('.wpb_el_type_lerp_matrix_set_amount button').on('click', function () {
                load_item($('input.loop_field[name="loop"]').val(), $('#lerp_matrix_items_container'));
         ...
         */
        if ($checkIfGallery == 1) {
            var $mainLoop = $('.lerp_gallery .lerp_gallery_attached_images_ids'),
                media = wp.media,
            ...
            $('li[data-tab-index="2"]').on('click', function (event) {
                $('#lerp_items_container, #lerp_matrix_items_container').empty();
            });
        ...
        function load_item($query, $container) {
            ...
            $templatePostObj['action'] = 'lerp_get_query';
        ...
        function init_single_item_list(html, $container) {
                ...
                for (var $index in $retrieveBundle) {
                    ...
                    if ($item.length != 0) {
                        ...
                        $('.option-tree-setting-reset', $item).unbind('*').on('click', function () {
                            ...
                            $('.lerp_bundle_items', $containerParent).val(Base64.encode(JSON.stringify($retrieveBundle)));
            ...
            /** fix custom link */
            $.each($('#lerp_items_container, #lerp_matrix_items_container').find('.wpb_el_type_vc_link'), function (index, val) {
                var $input = $('input', val),
                    $val = $input.val(),
            ...
            $.each($('#lerp_items_container, #lerp_matrix_items_container').find('.vc_control-media'), function (index, val) {
                refreshControlMedia(this, $retrieveBundle);
            });
            $.each($('#lerp_items_container, #lerp_matrix_items_container').find('.vc_control-text'), function (index, val) {
            ...
            $.each($('#lerp_items_container, #lerp_matrix_items_container').find('input.checkbox'), function (index, val) {
                $(this).removeAttr('id');
            ...
            /** fix iconpicker  **/
            $('#lerp_items_container, #lerp_matrix_items_container').find('> li .option-tree-setting-edit').bind('click', function () {
            ...
            $.each($('#lerp_items_container, #lerp_matrix_items_container').find('> li'), function (index, val) {
                var $liContent = $(val),
                    callDependencies = {};
                $.each(vc_user_mapper['lerp_index']['params'], function () {
                    var param = this;
        ...
        function refreshControlMedia(el, $retrieveBundle) {
            ...
            if ($mediaFound && $mediaFound != '') {
                var $div = '<div class="sorted-list-custom-media edit_form_line"><input type="hidden" class="wpb_vc_param_value lerp_gallery_attached_images_ids back_image media_element" name="back_image" value=""><div class="lerp_widget_attached_images"><ul class="lerp_widget_attached_images_list"></ul></div><div class="gallery_widget_site_images"></div><a class="add_media_widget button" href="#" use-single="true" title="Add media">Add media</a></div>';
                if ($mediaId != '' && $mediaId != undefined && $mediaFound) {
                    $.ajax({
                        ...
                        $mediaId = html;
                        $this.append('<div class="sorted-list-custom-media edit_form_line"><input type="hidden" class="wpb_vc_param_value lerp_gallery_attached_images_ids back_image media_element" name="back_image" value=""><div class="lerp_widget_attached_images"><ul class="lerp_widget_attached_images_list">' + $mediaId + '</ul></div><div class="gallery_widget_site_images"></div><a class="add_media_widget button" href="#" use-single="true" title="Add media">Add media</a></div>');
                    });
