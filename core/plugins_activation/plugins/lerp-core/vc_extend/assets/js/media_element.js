(function ($) {
    ...
    // ---------------------------------
    media.controller.lerpSingleMedia = media.controller.FeaturedImage.extend({
        defaults: _.defaults({
            id: 'lerp_single-media',
            filterable: 'uploaded',
            library: media.query(),
            multiple: false,
            toolbar: 'lerp_single-media',
        ...
        updateSelection: function () {
            var selection = this.get('selection'),
                id = media.lerpSingleMedia.getData(),
                attachment;
            if ('' !== id && -1 !== id) {
    ...
    media.controller.lerpMedia = media.controller.lerpSingleMedia.extend({
        defaults: _.defaults({
            id: 'lerp_media',
    ...
    media.view.uploadMediaView = media.View.extend({
        ...
        recordmedia: function () {
            media.lerp_editor.recordmedia();
        },
        ready: function () {
        },
    });
    media.lerpSingleMedia = {
        getData: function () {
            return this.$hidden_ids.val();
        },
        set: function (selection) {
            var template = _.template($('#lerp_settings-media-block').html());
        ...
        frame: function (element) {
            ...
            this.$hidden_ids = this.$block.find('.lerp_gallery_attached_images_ids');
            this.$img_ul = this.$block.find('.lerp_widget_attached_images_list');
            this.$clear_button = this.$img_ul.next();
            // TODO: Refactor this all params as template
            if (this._frame) return this._frame;
            this._frame = wp.media({
                state: 'lerp_single-media',
                states: [new wp.media.controller.lerpSingleMedia()]
            });
            this._frame.on('router:render:browse', this.browseRouter, this);
            this._frame.on('content:render:lerp', this.uploadMedia, this);
            this._frame.on('toolbar:create:lerp_single-media', function (toolbar) {
                this.createSelectToolbar(toolbar, {
                    text: l10n.add_media
                });
            }, this._frame);
            this._frame.on('toolbar:render:lerp_single-media', this.mainInsertToolbar, this);
            this._frame.state('lerp_single-media').on('select', this.select);
            return this._frame;
        },
        ...
        mainInsertToolbar: function (view) {
            ...
            view.set('select', {
                ...
                click: function () {
                    if (media.frame.content.get().el.className == 'uploader-lerp-media') {
                        media.lerp_editor.recordmedia();
                    } else {
                        $that.select();
                    }
        ...
        select: function () {
            ...
            media.lerpSingleMedia.set(selection ? selection : -1);
            media.frame.close();
        }
    };
    media.view.MediaFrame.lerpMedia = media.view.MediaFrame.Post.extend({
        ...
        createStates: function () {
            ...
            this.states.add([
                // Main states.
                new media.controller.lerpMedia(),
            ]);
        },
        ...
        mainInsertToolbar: function (view) {
            ...
            view.set('insert', {
                ...
                click: function () {
                    if (workflow.content.get().el.className == 'uploader-lerp-media') {
                        media.lerp_editor.recordmedia();
                    } else {
    ...
    media.lerp_editor = _.clone(media.editor);
    _.extend(media.lerp_editor, {
        $lerp_editor_element: null,
        getData: function () {
            var $button = media.lerp_editor.$lerp_editor_element,
                $block = $button.closest('.edit_form_line'),
                $hidden_ids = $block.find('.lerp_gallery_attached_images_ids');
        ...
        insert: function (images) {
            var $button = media.lerp_editor.$lerp_editor_element,
                $block = $button.closest('.edit_form_line'),
                $hidden_ids = $block.find('.lerp_gallery_attached_images_ids'),
                $img_ul = $block.find('.lerp_widget_attached_images_list'),
            ...
            if ($thumbnails_string != '') $('#lerp_gallery_div a.btn-remove-all').show();
        },
        open: function (id) {
            //var workflow, editor;
            id = this.id(id);
        ...
        add: function (id, options) {
            ...
            workflow = workflows[id] = new media.view.MediaFrame.lerpMedia(_.defaults(options || {}, {
                state: 'lerp_media',
        ...
        init: function () {
            $('body').unbind('click.lerpMediaWidget').on('click.lerpMediaWidget', '.add_media_widget', function (event) {
                ...
                var $this = addButton = $(this),
                    editor = 'lerp_editor';
                wp.media.lerp_editor.$lerp_editor_element = $(this);
                if ($this.attr('use-single') === 'true') {
                    wp.media.lerpSingleMedia.frame(this).open(editor);
                    return;
                }
                $this.blur();
                wp.media.lerp_editor.open(editor);
            });
        },
        get: function (id) {
            return workflows[id];
                ...
                success: function (data) {
                    if (!isNaN(data.id) && data != undefined) {
                        var $controller = $that.controller,
                            $addButton = media.lerp_editor.$lerp_editor_element,
    ...
    _.bindAll(media.lerp_editor, 'open');
    $(document).ready(function () {
        media.lerp_editor.init();
    });
    ...
    wp.media.view.Modal.prototype.on('open', function () {
        ...
        if (!$('body').hasClass('lerp-wp-media-with-galleries-open')) {
            option.remove();
        }
    });
    ...
    function addOpenMediaClass(show) {
        if (show) {
            $('body').addClass('lerp-wp-media-with-galleries-open');
        } else {
            $('body').removeClass('lerp-wp-media-with-galleries-open');
        }
    }
}(jQuery));
