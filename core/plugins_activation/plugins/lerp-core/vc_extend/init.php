...
function lerp_custom_css_classes_for_vc_row_and_vc_column($class_string, $tag)
{
...
add_filter('vc_shortcodes_css_class', 'lerp_custom_css_classes_for_vc_row_and_vc_column', 10, 2);

function lerp_init_front_custom_vc()
{
    ...
    add_action('wp_enqueue_scripts', 'lerp_dequeue_visual_composer');
    remove_action('wp_head', array(visual_composer(), 'addMetaData'));
    require_once 'config/lerp_map.php';
}

add_action('init', 'lerp_init_front_custom_vc', 1000);

function lerp_init_back_custom_vc()
{
...
add_action('admin_init', 'lerp_init_back_custom_vc');

function lerp_init_custom_js()
{
    wp_enqueue_style('vc-admin', plugins_url('assets/css/vc_admin.css', __FILE__), false, LerpCore_Plugin::VERSION);
...
add_action('vc_backend_editor_render', 'lerp_init_custom_js');

function lerp_hide_posts_not_published($query)
{
...
function lerp_index_get_query()
{
    $post_types = array();
    if ( function_exists('lerp_get_post_types') ) $post_types = lerp_get_post_types(true);
    ...
    add_filter('pre_get_posts', 'lerp_hide_posts_not_published');
    if ( $allItems === 'true' ) add_filter('pre_get_posts', 'lerp_all_posts');

    global $lerp_index_map;
    $u_index = new lerp_index($lerp_index_map);
        ...
        for ( $i = 0; $i < $matrix_amount; $i++ ) {
            ...
            $html .= '<li class="list-list-item" data-id="' . esc_attr($i) . '">
						<div class="option-tree-setting">
							<div class="open"><span class="post-title">' . sprintf(__('Item no. %s', 'lerp'), '<span class="matrix-item-no">' . esc_attr($i + 1) . '</span>') . '</span><span class="grey size">width: <span class="single_width_factor"></span>, height: <span class="single_height_factor"></span></span></div>
...
//For logged in users
add_action('wp_ajax_lerp_get_query', 'lerp_index_get_query');

function lerp_get_medias()
{
    $loop = vc_post_param('content');
    $template_single = vc_post_param('templateSingle');
...
add_action('wp_ajax_lerp_get_medias', 'lerp_get_medias');

/**
 * Suggestion list for wp_query field
 *
 */
class LerpLoopSuggestions
{
...
function lerp_get_loop_suggestion()
{
    $loop_suggestions = new LerpLoopSuggestions(vc_post_param('field'), vc_post_param('query'), vc_post_param('exclude'));
    $loop_suggestions->render();
    die();
}

add_action('wp_ajax_wpb_get_loop_suggestion', 'lerp_get_loop_suggestion');

function lerp_get_html()
{
...
add_action('wp_ajax_lerp_get_html', 'lerp_get_html');

function lerp_get_admin_media_post()
{
...
add_action('wp_ajax_lerp_get_media_post', 'lerp_get_admin_media_post');
...
class lerp_generic_admin extends WPBakeryShortCode
{
...
class lerp_row extends WPBakeryShortCode
{
...
class lerp_slider extends WPBakeryShortCode
{
...
class lerp_row_inner extends lerp_row
{
...
class lerp_slide extends lerp_row
{
    protected function getFileName()
    {
        return 'vc_row';
...
class lerp_counter extends WPBakeryShortCode
{
    public function template($content = '')
    {
        return $this
...
class lerp_countdown extends WPBakeryShortCode
{
    public function template($content = '')
    {
        return $this
...
class lerp_list extends WPBakeryShortCode
{
    public function template($content = '')
    {
        return $this
...
class lerp_pricing extends WPBakeryShortCode
{
    public function template($content = '')
    {
        return $this
...
class lerp_share extends WPBakeryShortCode
{
    public function template($content = '')
    {
        return $this
...
class lerp_twentytwenty extends WPBakeryShortCode
{
    public function template($content = '')
    {
        return $this
...
class lerp_block extends WPBakeryShortCode
{
    public function template($content = '')
    {
        return $this
...
class lerp_author_profile extends WPBakeryShortCode
{
    public function template($content = '')
    {
        return $this
...
class lerp_index extends WPBakeryShortCode
{
    ...
    public function buildWpQuery($query, $exclude_id = false, $offset = false)
    {
        ...
        $query_builder = new LerpLoopQueryBuilder($data);
        if ( $exclude_id ) {
...
class LerpLoopQueryBuilder extends VcLoopQueryBuilder
{
...
add_filter('vc_iconpicker-type-lerp', 'lerp_vc_iconpicker_type_lerp');

function lerp_vc_iconpicker_type_lerp($icons)
{
    $lerp_icons = array();
    $lerp_icons[] = array('' => '');
    ...
    foreach ( $icons_file['icons'] as $key => $value ) {
        ...
        $lerp_icons[] = array('fa fa-' . $names[0] => ucwords(implode(', ', $value['icon']['tags'])));
    }

    return array_merge($icons, $lerp_icons);
}
...
function lerp_replace_inner_single_width($content, $width_media)
{
...
/**
 * Set lerp_gallery as available CPT for VC.
 */
// function lerp_set_vc_editor_cpts() {
// 	if ( function_exists( ( 'vc_editor_post_types' ) ) && function_exists( ( 'vc_editor_set_post_types' ) ) && get_option('lerp_gallery_installed') != true ) {
// 		$saved_post_types = vc_editor_post_types();

// 		if ( ! in_array( 'lerp_gallery', $saved_post_types ) ) {
// 			$saved_post_types[] = 'lerp_gallery';
// 			vc_editor_set_post_types( $saved_post_types );
// 			update_option('lerp_gallery_installed', true);
// 		}
// 	}
// }
// add_action('admin_init', 'lerp_set_vc_editor_cpts');
