                    ...
                    array(
                        ...
                        'pages' => array(
                            array(
                                ...
                                'page_title' => apply_filters('ot_theme_options_page_title',
                                    esc_html__('Theme Options', 'option-tree')),
                                'menu_title' => apply_filters('ot_theme_options_menu_title',
                                    esc_html__('Theme Options', 'option-tree')),
                                'capability' => $caps = apply_filters('ot_theme_options_capability',
                                    'edit_theme_options'),
                                ...
                                'updated_message' => apply_filters('ot_theme_options_updated_message',
                                    esc_html__('Theme Options updated.', 'option-tree')),
                                'reset_message' => apply_filters('ot_theme_options_reset_message',
                                    esc_html__('Theme Options reset.', 'option-tree')),
                                'button_text' => apply_filters('ot_theme_options_button_text',
                                    esc_html__('Save Changes', 'option-tree')),
                                'contextual_help' => apply_filters('ot_theme_options_contextual_help',
                                    $contextual_help),
...
if ( !function_exists('ot_register_settings_page') ) {
    ...
    function ot_register_settings_page()
    {
        ...
        if ( OT_SHOW_OPTIONS_UI == true && isset($_REQUEST['page']) && $_REQUEST['page'] == 'lerp-settings' && ($ot_has_custom_theme_options == true || has_action('admin_init',
                    'custom_theme_options') || has_action('init', 'custom_theme_options')) ) {

            function ot_has_custom_theme_options()
            {

                echo '<div class="error"><p>' . esc_html__('The Theme Options UI Builder is being overridden by a custom file in your theme. Any changes you make via the UI Builder will not be saved.',
                        'option-tree') . '</p></div>';
            ...
            array(
                ...
                'settings' => array(
                    ...
                    array(
                        ...
                        'label' => esc_html__('Option types in alphabetical order & hooks to filter them.',
                            'option-tree'),
                        'type' => 'option-types',
        ...
        if ( 'background' == $type ) {
            ...
            foreach ( $input as $key => $value ) {
                ...
                if ( $key == 'width' && !empty($value) && !is_numeric($value) ) {
                    ...
                    add_settings_error('option-tree',
                        'invalid_border_width',
                        sprintf(wp_kses(__('The %s input field for %s only allows numeric values.', 'option-tree'),
                            array('code' => array())),
                            '<code>width</code>',
                            '<code>' . $field_id . '</code>'),
                        'error');
            ...
            if ( 0 === preg_match('/^#([a-f0-9]{6}|[a-f0-9]{3})$/i',
                    $input) && 0 === preg_match('/^rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9\.]{1,4})\s*\)/i',
                    $input) ) {
                ...
                add_settings_error('option-tree',
                    'invalid_hex',
                    sprintf(wp_kses(__('The %s Colorpicker only allows valid hexadecimal or rgba values.',
                        'option-tree'),
                        array('code' => array())),
                        '<code>' . $field_id . '</code>'),
                    'error');
...
if ( !function_exists('ot_admin_styles') ) {
    ...
    function ot_admin_styles()
    {
         ...
         */
        $screen_ids = apply_filters('ot_dequeue_jquery_ui_css_screen_ids',
            array(
...
if ( !function_exists('ot_admin_scripts') ) {
    ...
    function ot_admin_scripts()
    {
        ...
        if ( OT_META_BOXES == true && OT_POST_FORMATS == true ) {
            wp_enqueue_script('ot-postformats',
                OT_URL . 'assets/js/min/ot-postformats.min.js',
                array('jquery'),
                '1.0.1');
        }

        /* load all the required scripts */
        wp_enqueue_script('ot-admin-js',
            OT_URL . 'assets/js/min/ot-admin.min.js',
...
if ( !function_exists('ot_create_media_post') ) {
    ...
    function ot_create_media_post()
    {
        ...
        $regsiter_post_type('option-tree',
            array(
    ...
    function ot_default_settings()
    {
        ...
        if ( !get_option(ot_settings_id()) ) {
            ...
            if ( $wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s",
                    $table_name)) == $table_name && $old_settings = $wpdb->get_results("SELECT * FROM $table_name ORDER BY item_sort ASC") ) {
...
if ( !function_exists('ot_import') ) {
    ...
    function ot_import()
    {
        ...
        if ( isset($_POST['import_xml_nonce']) && wp_verify_nonce($_POST['import_xml_nonce'], 'import_xml_form') ) {
            ...
            /* redirect */
            wp_redirect(esc_url_raw(add_query_arg(array('action' => 'import-xml', 'message' => $message),
        ...
        if ( isset($_POST['import_settings_nonce']) && wp_verify_nonce($_POST['import_settings_nonce'],
            ...
            /* redirect */
            wp_redirect(esc_url_raw(add_query_arg(array('action' => 'import-settings', 'message' => $message),
                $_POST['_wp_http_referer'])));
            exit;
        ...
        if ( isset($_POST['import_data_nonce']) && wp_verify_nonce($_POST['import_data_nonce'], 'import_data_form') ) {
            ...
            wp_redirect(esc_url_raw(add_query_arg(array('action' => 'import-data', 'message' => $message),
                $_POST['_wp_http_referer'])));
            exit;
        ...
        if ( isset($_POST['import_layouts_nonce']) && wp_verify_nonce($_POST['import_layouts_nonce'],
                    ...
                    foreach ( $layouts as $key => $value ) {
                        ...
                        foreach ( $settings['settings'] as $setting ) {
                            ...
                            if ( isset($options[$setting['id']]) ) {
                                ...
                                $options[$setting['id']] = ot_validate_setting($content,
                                    $setting['type'],
                                    $setting['id']);
            ...
            /* redirect accordingly */
            wp_redirect(esc_url_raw(add_query_arg(array('action' => 'import-layouts', 'message' => $message),
                $_POST['_wp_http_referer'])));
            exit;
    ...
    function ot_export_php_settings_array()
    {
        ...
        if ( isset($option_tree_settings['contextual_help']['content']) ) {
            ...
            foreach ( $option_tree_settings['contextual_help']['content'] as $value ) {
                ...
                $_content = ot_I18n_string(isset($value['content']) ? html_entity_decode(str_replace("'",
                    "\'",
                    $value['content'])) : '');
                $help .= "
				array(
        ...
        if ( isset($option_tree_settings['contextual_help']['sidebar']) ) {
            $contextual_help .= "
			'sidebar'       => " . ot_I18n_string(html_entity_decode(str_replace("'",
                    "\'",
            ...
            foreach ( $option_tree_settings['settings'] as $value ) {
                ...
                if ( isset($value['choices']) && !empty($value['choices']) ) {
                    foreach ( $value['choices'] as $choice ) {
                        ...
                        $_choice_label = ot_I18n_string(isset($choice['label']) ? str_replace("'",
                            "\'",
                            $choice['label']) : '');
...
' . implode(",\n", $std_array) . '
                ...
                if ( isset($value['settings']) && !empty($value['settings']) ) {
                    foreach ( $value['settings'] as $setting ) {
                        ...
                        $_setting_label = ot_I18n_string(isset($setting['label']) ? str_replace("'",
                            "\'",
                            $setting['label']) : '');
                        $_setting_desc = ot_I18n_string(isset($setting['desc']) ? str_replace("'",
                            "\'",
                            $setting['desc']) : '');
                        ...
                        if ( isset($setting['choices']) && !empty($setting['choices']) ) {
                            foreach ( $setting['choices'] as $setting_choice ) {
                                ...
                                $_setting_choice_label = ot_I18n_string(isset($setting_choice['label']) ? str_replace("'",
                                    "\'",
...
if ( !function_exists('ot_save_settings') ) {
    ...
    function ot_save_settings()
    {
        ...
        if ( isset($_POST['option_tree_settings_nonce']) && wp_verify_nonce($_POST['option_tree_settings_nonce'],
            ...
            /* redirect */
            wp_redirect(esc_url_raw(add_query_arg(array('action' => 'save-settings', 'message' => $message),
                $_POST['_wp_http_referer'])));
            exit;
    ...
    function ot_modify_layouts()
    {
        ...
        if ( isset($_POST['option_tree_modify_layouts_nonce']) && wp_verify_nonce($_POST['option_tree_modify_layouts_nonce'],
            ...
            if ( isset($_REQUEST['page']) && $_REQUEST['page'] == apply_filters('ot_theme_options_menu_slug',
                    'ot-theme-options') ) {
                $query_args = esc_url_raw(add_query_arg(array('settings-updated' => 'layout'),
                    remove_query_arg(array('action', 'message'), $_POST['_wp_http_referer'])));
            } else {
                $query_args = esc_url_raw(add_query_arg(array('action' => 'save-layouts', 'message' => $message),
                    $_POST['_wp_http_referer']));
            }
    ...
    function ot_alert_message($page = array())
    {
        ...
        if ( $action == 'save-settings' ) {
            ...
            if ( $message == 'success' ) {
                ...
                if ( $page['id'] == 'settings' ) {
                    return '<div class="lerp-ui-notice lerp-ui-notice--success fade"><p>' . esc_html__('Settings updated.',
                            'option-tree') . '</p></div>';
                } else {
                    return '<div id="lerp-ot-message" class="updated fade below-h2"><p>' . esc_html__('Settings updated.',
                            'option-tree') . '</p></div>';
                }
                ...
                if ( $page['id'] == 'settings' ) {
                    return '<div class="lerp-ui-notice lerp-ui-notice--error fade"><p>' . esc_html__('Settings could not be saved.',
                            'option-tree') . '</p></div>';
                } else {
                    return '<div id="lerp-ot-message" class="error fade below-h2"><p>' . esc_html__('Settings could not be saved.',
                            'option-tree') . '</p></div>';
                }
            ...
            if ( $message == 'success' ) {
                ...
                if ( $page['id'] == 'settings' ) {
                    return '<div class="lerp-ui-notice lerp-ui-notice--success fade"><p>' . esc_html__('Settings Imported.',
                            'option-tree') . '</p></div>';
                } else {
                    return '<div id="lerp-ot-message" class="updated fade below-h2"><p>' . esc_html__('Settings Imported.',
                            'option-tree') . '</p></div>';
                }
                ...
                if ( $page['id'] == 'settings' ) {
                    return '<div class="lerp-ui-notice lerp-ui-notice--error fade"><p>' . esc_html__('Settings could not be imported.',
                            'option-tree') . '</p></div>';
                } else {
                    return '<div id="lerp-ot-message" class="error fade below-h2"><p>' . esc_html__('Settings could not be imported.',
                            'option-tree') . '</p></div>';
                }
            ...
            if ( $message == 'success' ) {
                ...
                if ( $page['id'] == 'settings' ) {
                    return '<div class="lerp-ui-notice lerp-ui-notice--success fade"><p>' . esc_html__('Data Imported.',
                            'option-tree') . '</p></div>';
                } else {
                    return '<div id="lerp-ot-message" class="updated fade below-h2"><p>' . esc_html__('Data Imported.',
                            'option-tree') . '</p></div>';
                }
                ...
                if ( $page['id'] == 'settings' ) {
                    return '<div class="lerp-ui-notice lerp-ui-notice--error fade"><p>' . esc_html__('Data could not be imported.',
                            'option-tree') . '</p></div>';
                } else {
                    return '<div id="lerp-ot-message" class="error fade below-h2"><p>' . esc_html__('Data could not be imported.',
                            'option-tree') . '</p></div>';
                }
            ...
            if ( $message == 'success' ) {
                ...
                if ( $page['id'] == 'settings' ) {
                    return '<div class="lerp-ui-notice lerp-ui-notice--success fade"><p>' . esc_html__('Layouts Imported.',
                            'option-tree') . '</p></div>';
                } else {
                    return '<div id="lerp-ot-message" class="updated fade below-h2"><p>' . esc_html__('Layouts Imported.',
                            'option-tree') . '</p></div>';
                }
                ...
                if ( $page['id'] == 'settings' ) {
                    return '<div class="lerp-ui-notice lerp-ui-notice--error fade"><p>' . esc_html__('Layouts could not be imported.',
                            'option-tree') . '</p></div>';
                } else {
                    return '<div id="lerp-ot-message" class="error fade below-h2"><p>' . esc_html__('Layouts could not be imported.',
                            'option-tree') . '</p></div>';
                }
            ...
            if ( $message == 'success' ) {
                ...
                if ( $page['id'] == 'settings' ) {
                    return '<div class="lerp-ui-notice lerp-ui-notice--success fade"><p>' . esc_html__('Layouts Updated.',
                            'option-tree') . '</p></div>';
                } else {
                    return '<div id="lerp-ot-message" class="updated fade below-h2"><p>' . esc_html__('Layouts Updated.',
                            'option-tree') . '</p></div>';
                }
                ...
                if ( $page['id'] == 'settings' ) {
                    return '<div class="lerp-ui-notice lerp-ui-notice--error fade"><p>' . esc_html__('Layouts could not be updated.',
                            'option-tree') . '</p></div>';
                } else {
                    return '<div id="lerp-ot-message" class="error fade below-h2"><p>' . esc_html__('Layouts could not be updated.',
                            'option-tree') . '</p></div>';
                }
                ...
                if ( $page['id'] == 'settings' ) {
                    return '<div class="lerp-ui-notice lerp-ui-notice--success fade"><p>' . esc_html__('Layouts have been deleted.',
                            'option-tree') . '</p></div>';
                } else {
                    return '<div id="lerp-ot-message" class="updated fade below-h2"><p>' . esc_html__('Layouts have been deleted.',
                            'option-tree') . '</p></div>';
                }
            ...
            if ( $page['id'] == 'settings' ) {
                return '<div class="lerp-ui-notice lerp-ui-notice--success fade"><p>' . esc_html__('Layout activated.',
                        'option-tree') . '</p></div>';
            } else {
                return '<div id="lerp-ot-message" class="updated fade below-h2"><p>' . esc_html__('Layout activated.',
                        'option-tree') . '</p></div>';
            }
...
if ( !function_exists('ot_recognized_border_style_types') ) {
    ...
    function ot_recognized_border_style_types($field_id = '')
    {
        ...
        return apply_filters('ot_recognized_border_style_types',
            array(
                'hidden' => 'Hidden',
                'dashed' => 'Dashed',
                'solid' => 'Solid',
...
if ( !function_exists('ot_recognized_border_unit_types') ) {
    ...
    function ot_recognized_border_unit_types($field_id = '')
    {
        ...
        return apply_filters('ot_recognized_border_unit_types',
            array(
                'px' => 'px',
                '%' => '%',
                'em' => 'em',
...
if ( !function_exists('ot_recognized_dimension_unit_types') ) {
    ...
    function ot_recognized_dimension_unit_types($field_id = '')
    {
        ...
        return apply_filters('ot_recognized_dimension_unit_types',
            array(
                'px' => 'px',
                '%' => '%',
                'em' => 'em',
...
if ( !function_exists('ot_recognized_spacing_unit_types') ) {
    ...
    function ot_recognized_spacing_unit_types($field_id = '')
    {
        ...
        return apply_filters('ot_recognized_spacing_unit_types',
            array(
                'px' => 'px',
                '%' => '%',
                'em' => 'em',
        ...
        $settings = apply_filters('ot_social_links_settings',
            array(
                ...
                array(
                    'id' => 'href',
                    'label' => 'Link',
                    'desc' => sprintf(wp_kses(__('Enter a link to the profile or page on the social website. Remember to add the %s part to the front of the link.',
                        'option-tree'),
                        array('code' => array())),
                        '<code>http://</code>'),
                    'type' => 'text',
        ...
        if ( file_exists($filepath) ) {
            ...
            foreach ( $matches[0] as $option ) {
                        ...
                        if ( $option_type == 'measurement' ) {
                            ...
                            if ( !empty($value['background-image']) ) {
                                ...
                                if ( wp_attachment_is_image($value['background-image']) ) {
                                    ...
                                    $attachment_data = wp_get_attachment_image_src($value['background-image'],
                                        'original');
                ...
                if ( empty($value) ) {
                     ...
                     */
                    $fallback = apply_filters('ot_insert_css_with_markers_fallback',
                        $fallback,
                        $option_id,
                        $option_type,
                        $option_key);
            ...
            if ( !is_writable($filepath) ) {
                add_settings_error('option-tree',
                    'dynamic_css',
                    sprintf(wp_kses(__('Unable to write to file %s.', 'option-tree'), array('code' => array())),
                        '<code>' . $filepath . '</code>'),
                    'error');
                return false;
            }
...
if ( !function_exists('ot_loop_through_sub_settings') ) {
    ...
    function ot_loop_through_sub_settings($name, $settings = array())
    {
        ...
        foreach ( $settings as $key => $setting )
            $content .= '<li class="ui-state-default list-sub-setting">' . ot_settings_view($name,
                    $key,
                    $setting) . '</li>';
    ...
    function ot_sections_view($name, $key, $section = array())
    {
		...
		<div class="option-tree-setting is-section">
			...
			<div class="button-section">
				<a href="javascript:void(0);" class="option-tree-setting-edit option-tree-ui-button button left-item" title="' . esc_html__('edit',
                'option-tree') . '">
					<span class="icon ot-icon-pencil"></span>' . esc_html__('Edit', 'option-tree') . '
				</a>
				<a href="javascript:void(0);" class="option-tree-setting-remove option-tree-ui-button button button-secondary light right-item" title="' . esc_html__('Delete',
                'option-tree') . '">
			...
			<div class="option-tree-setting-body">
				<div class="format-settings">
					<div class="format-setting type-text">
						<div class="description">' . esc_html__('<strong>Section Title</strong>: Displayed as a menu item on the Theme Options page.',
                'option-tree') . '</div>
				...
				<div class="format-settings">
					<div class="format-setting type-text">
						<div class="description">' . esc_html__('<strong>Section ID</strong>: A unique lower case alphanumeric string, underscores allowed.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
			...
			<div class="option-tree-setting-body">
				<div class="format-settings">
					<div class="format-setting type-text wide-desc">
						<div class="description">' . esc_html__('<strong>Label</strong>: Displayed as the label of a form element on the Theme Options page.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
				...
				<div class="format-settings">
					<div class="format-setting type-text wide-desc">
						<div class="description">' . esc_html__('<strong>ID</strong>: A unique lower case alphanumeric string, underscores allowed.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
				...
				<div class="format-settings">
					<div class="format-setting type-select wide-desc">
						<div class="description">' . esc_html__('<strong>Type</strong>: Choose one of the available option types from the dropdown.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
				...
				<div class="format-settings">
					<div class="format-setting type-textarea wide-desc">
						<div class="description">' . esc_html__('<strong>Description</strong>: Enter a detailed description for the users to read on the Theme Options page, HTML is allowed. This is also where you enter content for both the Textblock & Textblock Titled option types.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
				...
				<div class="format-settings">
					<div class="format-setting type-textblock wide-desc">
						<div class="description">' . esc_html__('<strong>Choices</strong>: This will only affect the following option types: Checkbox, Radio, Select & Select Image.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
                ...
                $setting['choices']) : '') . '
							...
							<a href="javascript:void(0);" class="option-tree-choice-add option-tree-ui-button button hug-left">' . esc_html__('Add Choice',
                'option-tree') . '</a>
						</div>
					</div>
				</div>
				<div class="format-settings">
					<div class="format-setting type-textblock wide-desc">
						<div class="description">' . esc_html__('<strong>Settings</strong>: This will only affect the List Item option type.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
							<ul class="option-tree-setting-wrap option-tree-sortable" data-name="' . esc_attr($name) . '[' . esc_attr($key) . ']">
                ...
                $setting['settings']) : '') . '
							...
							<a href="javascript:void(0);" class="option-tree-list-item-setting-add option-tree-ui-button button hug-left">' . esc_html__('Add Setting',
                'option-tree') . '</a>
						</div>
					</div>
				</div>
				<div class="format-settings">
					<div class="format-setting type-text wide-desc">
						<div class="description">' . esc_html__('<strong>Standard</strong>: Setting the standard value for your option only works for some option types. Read the <code>OptionTree->Documentation</code> for more information on which ones.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
							' . $std_form_element . '
						</div>
				...
				<div class="format-settings">
					<div class="format-setting type-text wide-desc">
						<div class="description">' . esc_html__('<strong>Rows</strong>: Enter a numeric value for the number of rows in your textarea. This will only affect the following option types: CSS, Textarea, & Textarea Simple.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
				...
				<div class="format-settings">
					<div class="format-setting type-text wide-desc">
						<div class="description">' . esc_html__('<strong>Post Type</strong>: Add a comma separated list of post type like \'post,page\'. This will only affect the following option types: Custom Post Type Checkbox, & Custom Post Type Select.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
				...
				<div class="format-settings">
					<div class="format-setting type-text wide-desc">
						<div class="description">' . esc_html__('<strong>Taxonomy</strong>: Add a comma separated list of any registered taxonomy like \'category,post_tag\'. This will only affect the following option types: Taxonomy Checkbox, & Taxonomy Select.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
				...
				<div class="format-settings">
					<div class="format-setting type-text wide-desc">
						<div class="description">' . esc_html__('<strong>CSS Class</strong>: Add and optional class to this option type.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
				...
				<div class="format-settings">
					<div class="format-setting type-text wide-desc">
						<div class="description">' . sprintf(wp_kses(__('<strong>Condition</strong>: Add a comma separated list (no spaces) of conditions in which the field will be visible, leave this setting empty to always show the field. In these examples, <code>value</code> is a placeholder for your condition, which can be in the form of %s.',
                'option-tree'),
                array('code' => array(), 'strong' => array())),
                '<code>field_id:is(value)</code>, <code>field_id:not(value)</code>, <code>field_id:contains(value)</code>, <code>field_id:less_than(value)</code>, <code>field_id:less_than_or_equal_to(value)</code>, <code>field_id:greater_than(value)</code>, or <code>field_id:greater_than_or_equal_to(value)</code>') . '</div>
						<div class="format-setting-inner">
				...
				<div class="format-settings">
					<div class="format-setting type-select wide-desc">
						<div class="description">' . esc_html__('<strong>Operator</strong>: Choose the logical operator to compute the result of the conditions.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
							<select name="' . esc_attr($name) . '[' . esc_attr($key) . '][operator]" value="' . $operator . '" class="option-tree-ui-select">
								<option value="and" ' . selected($operator, 'and', false) . '>' . esc_html__('and',
                'option-tree') . '</option>
								<option value="or" ' . selected($operator, 'or', false) . '>' . esc_html__('or',
                'option-tree') . '</option>
							</select>
						</div>
					</div>
			...
			<div class="option-tree-setting-body">
				<div class="format-settings">
					<div class="format-setting type-text no-desc">
						<div class="description">' . esc_html__('<strong>Title</strong>: Displayed as a contextual help menu item on the Theme Options page.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
				...
				<div class="format-settings">
					<div class="format-setting type-textarea no-desc">
						<div class="description">' . esc_html__('<strong>Content</strong>: Enter the HTML content about this contextual help item displayed on the Theme Option page for end users to read.',
                'option-tree') . '</div>
						<div class="format-setting-inner">
...
if ( !function_exists('ot_layout_view') ) {
    ...
    function ot_layout_view($key, $data = '', $active_layout = '')
    {
		...
		<div class="option-tree-setting">
			...
			<div class="button-section">
				<a href="javascript:void(0);" class="option-tree-layout-activate option-tree-ui-button button left-item' . ($active_layout == $key ? ' active' : '') . '" title="' . esc_html__('Activate',
                'option-tree') . '">
...
if ( !function_exists('ot_list_item_view') ) {
    ...
    function ot_list_item_view($name,
                               $key,
                               $list_item = array(),
                               $post_id = 0,
                               $get_option = '',
        ...
        foreach ( $settings as $field ) {
            ...
            if ( isset($field['condition']) && !empty($field['condition']) ) {
                ...
                foreach ( $conditionals as $condition ) {
                    ...
                    if ( isset($parts[0]) ) {
                        $field['condition'] = str_replace($condition,
                            $name . '_' . $parts[0] . '_' . $key . ':' . $parts[1],
                            $field['condition']);
                    }
                }
                ...
                $conditions .= isset($field['operator']) && in_array($field['operator'],
                    array('and', 'AND', 'or', 'OR')) ? ' data-operator="' . $field['operator'] . '"' : '';
            ...
            if ( apply_filters('ot_override_forced_textarea_simple',
                    false,
                    $field['id']) == false && $_args['type'] == 'textarea' )
...
if ( !function_exists('ot_social_links_view') ) {
    ...
    function ot_social_links_view($name,
                                  $key,
                                  $list_item = array(),
                                  $post_id = 0,
                                  $get_option = '',
...
function ot_fetch_google_fonts($normalize = true, $force_rebuild = false)
{
    ...
    if ( $force_rebuild || !is_array($ot_google_fonts) || empty($ot_google_fonts) ) {
        ...
        /* API url and key */
        $ot_google_fonts_api_url = apply_filters('ot_google_fonts_api_url',
        ...
        $ot_google_fonts_response = wp_safe_remote_get($ot_google_fonts_query,
        ...
        array(
            ...
            'fields' => array(
                array(
                    ...
                    'label' => '',
                    'desc' => sprintf(wp_kses(__('Embed video from services like Youtube, Vimeo, or Hulu. You can find a list of supported oEmbed sites in the %1$s. Alternatively, you could use the built-in %2$s shortcode.',
                        'option-tree'),
                        array('code' => array(), 'a' => array('href' => array(), 'target' => array()))),
                        '<a href="http://codex.wordpress.org/Embeds" target="_blank">' . esc_html__('WordPress Codex',
                            'option-tree') . '</a>',
                        '<code>[video]</code>'),
                    'std' => '',
                    'type' => 'textarea'
        ...
        array(
            ...
            'fields' => array(
                array(
                    ...
                    'label' => '',
                    'desc' => sprintf(wp_kses(__('Embed audio from services like SoundCloud and Rdio. You can find a list of supported oEmbed sites in the %1$s. Alternatively, you could use the built-in %2$s shortcode.',
                        'option-tree'),
                        array('code' => array(), 'a' => array('href' => array(), 'target' => array()))),
                        '<a href="http://codex.wordpress.org/Embeds" target="_blank">' . esc_html__('WordPress Codex',
                            'option-tree') . '</a>',
                ...
                if ( isset($value['type']) ) {
                    ...
                    if ( $value['type'] == 'list-item' && isset($value['settings']) ) {
                        ...
                        foreach ( $value['settings'] as $item ) {
                            ...
                            if ( isset($value['id']) && isset($item['type']) && in_array($item['type'],
                                    $option_types) ) {
...
function ot_split_shared_term($term_id, $new_term_id, $term_taxonomy_id, $taxonomy)
{
    ...
    if ( !empty($settings) && !empty($old_options) ) {
        ...
        foreach ( $settings as $option ) {
            ...
            if ( array_key_exists($option['id'],
                    $old_options) || (isset($option['parent']) && array_key_exists($option['parent'], $old_options)) ) {
