...
function custom_theme_options()
{
	...
	if (is_plugin_active('lerp-core/lerp-core.php')) {
		...
		$lerpblocks = array(
			array(
				'value' => '','label' => esc_html__('Inherit', 'lerp')
			),
			array(
				'value' => 'none','label' => esc_html__('None', 'lerp')
			)
		);
	...
	$menu_section_title = array(
		...
		'label' => ' <i class="fa fa-menu"></i> ' . esc_html__('Menu', 'lerp') ,
		'desc' => '' ,
	...
	$menu_opaque = array(
		...
		'type' => 'on-off',
		'desc' => esc_html__('Override to remove the transparency eventually declared in \'Customize -> Light/Dark skin\'.', 'lerp') ,
		'std' => 'off',
		'section' => 'lerp_%section%_section',
	);
	...
	$header_title_text = array(
		...
		'desc' => esc_html__('Add custom text for the header. Every newline in the field is a new line in the title.', 'lerp') ,
	...
	$body_lerp_block = array(
		...
		'label' => esc_html__('Content Block', 'lerp') ,
		'desc' => esc_html__('Define the Content Block to use. NB. Select "Inherit" to use the default template.', 'lerp') ,
		'type' => 'select',
		'choices' => $lerpblocks,
		'section' => 'lerp_%section%_section',
	);
	...
	$body_layout_width_custom = array(
		...
		'desc' => esc_html__('Define the custom width for the content area in px or in %. This option takes effect with normal contents (not Page Builder).', 'lerp') ,
		'type' => 'measurement',
	...
	$body_single_text_lenght = array(
		...
		'desc' => esc_html__('Enter the number of words you want for the teaser. If nothing in entered the full content will be showed.', 'lerp') ,
		'type' => 'text',
		'section' => 'lerp_%section%_section',
		'condition' => '_lerp_%section%_content_block:is(),_lerp_%section%_content_block:is(none)',
		'operator' => 'or',
	...
	$custom_settings_one = array(
		...
		array(
			'id' => '_lerp_main_width',
			'label' => esc_html__('Site width', 'lerp') ,
			'desc' => esc_html__('Enter the width of your site.', 'lerp') ,
			'std' => array(
				'1200',
				'px'
			) ,
		...
		array(
			'id' => '_lerp_boxed',
			'label' => esc_html__('Boxed', 'lerp') ,
			'desc' => esc_html__('Activate for the boxed layout.', 'lerp') ,
			'std' => 'off',
			'type' => 'on-off',
			'section' => 'lerp_main_section',
		) ,
		...
		array(
			...
			'desc' => esc_html__('Activate this for the shadow effect on menu dropdown on desktop view. NB. this option works for horizontal menus only.', 'lerp') ,
			'std' => 'none',
			'type' => 'select',
			'section' => 'lerp_header_section',
			'std' => '',
		...
		array(
			...
			'desc' => esc_html__('Activate the sticky menu. This is a menu that is locked into place so that it does not disappear when the user scrolls down the page.', 'lerp') ,
			'std' => 'on',
			'type' => 'on-off',
			'section' => 'lerp_header_section',
		...
		array(
			...
			'desc' => esc_html__('Activate the sticky menu on mobile devices. This is a menu that is locked into place so that it does not disappear when the user scrolls down the page.', 'lerp') ,
			'std' => 'off',
			'type' => 'on-off',
			'section' => 'lerp_header_section',
		) ,
		array(
			...
			'desc' => esc_html__('Activate the centered style for mobile menu. NB. You need to have the Menu Sticky Mobile active.', 'lerp') ,
			'std' => 'off',
			'type' => 'on-off',
			'section' => 'lerp_header_section',
		...
		array(
			...
			'desc' => esc_html__('Activate the autohide menu. This is a menu that is hiding after the user have scrolled down the page.', 'lerp') ,
			'std' => 'on',
			'type' => 'on-off',
			'section' => 'lerp_header_section',
		...
		array(
			...
			'desc' => esc_html__('Activate the autohide menu on mobile devices. This is a menu that is hiding after the user have scrolled down the page.', 'lerp') ,
			'std' => 'off',
			'type' => 'on-off',
			'section' => 'lerp_header_section',
		) ,
		array(
			...
			'desc' => esc_html__('Activate the shrink menu. This is a menu where the logo shrinks after the user have scrolled down the page.', 'lerp') ,
			'std' => 'off',
			'type' => 'on-off',
			'section' => 'lerp_header_section',
			'condition' => '_lerp_headers:contains(hmenu),_lerp_headers:is(menu-overlay),_lerp_headers:is(menu-overlay-center),_lerp_headers:is(vmenu-offcanvas)',
		...
		array(
			...
			'desc' => esc_html__('Activate the animation for menu sub-levels. NB. this option works for horizontal menus only.', 'lerp') ,
			'std' => 'off',
			'type' => 'on-off',
			'section' => 'lerp_header_section',
		...
		array(
			...
			'desc' => esc_html__('Show the cart icon in the menu bar when layout is on desktop mode (Only for overlay and offcanvas menu).', 'lerp') ,
			'std' => 'off',
			'type' => 'on-off',
			'section' => 'lerp_header_section',
	...
	$custom_settings_two = array(
		...
		str_replace('%section%', 'page_index', run_array_to($body_layout_width, 'condition', '_lerp_%section%_content_block:is(),_lerp_%section%_content_block:is(none)')),
	...
	$custom_settings_three = array(
		...
		array(
			...
			'desc' => esc_html__('Define here all the sidebars you will need. A default sidebar is already defined.', 'lerp') ,
			'type' => 'list-item',
		...
		array(
			...
			'settings' => array(
				...
				array(
					...
					'desc' => esc_html__('Activate to assign a monochromatic color, otherwise a gradient will be used.', 'lerp') ,
					'std' => 'on',
					'type' => 'on-off',
				...
				array(
					...
					'desc' => esc_html__('Specify the color for this palette. You can also define a color with the alpha value.', 'lerp') ,
					'std' => '',
					'type' => 'colorpicker',
				...
				array(
					...
					'desc' => esc_html__('Specify the gradient color for this palette. NB. You can use a gradient color only as a background color.', 'lerp') ,
					'std' => '',
					'type' => 'gradientpicker',
		...
		array(
			...
			'desc' => esc_html__('Specify the menu active and hover effect color (If not specified an opaque version of the menu color will be used).', 'lerp') ,
			'std' => '',
			'type' => 'lerp_color',
			'section' => 'lerp_customize_section',
		) ,
		...
		array(
			...
			'desc' => esc_html__('Specify the menu font size. NB: the Overlay menu font size is automatic relative to the viewport.', 'lerp') ,
			'std' => '12',
			'type' => 'text',
			'section' => 'lerp_customize_section',
		) ,
		array(
			...
			'desc' => esc_html__('Specify the submenu font size. NB: the Overlay submenu font size is automatic relative to the viewport.', 'lerp') ,
			'std' => '12',
			'type' => 'text',
			'section' => 'lerp_customize_section',
		) ,
	...
	$custom_settings_four = array(
		...
		array(
			...
			'label' => '<i class="fa fa-layers"></i> ' . esc_html__('Layout', 'lerp') ,
			'type' => 'textblock-titled',
			'class' => 'section-title',
		...
		array(
			...
			'label' => esc_html__('Footer full width', 'lerp') ,
			'desc' => esc_html__('Expand the footer to full width.', 'lerp') ,
			'std' => 'off',
			'type' => 'on-off',
			'section' => 'lerp_footer_section',
			'condition' => '_lerp_boxed:is(off)',
		...
		array(
			...
			'label' => '<i class="fa fa-cog2"></i> ' . esc_html__('Widget area', 'lerp') ,
			'type' => 'textblock-titled',
			'class' => 'section-title',
			'section' => 'lerp_footer_section',
		) ,
		array(
			...
			'label' => esc_html__('Content Block', 'lerp') ,
			'desc' => esc_html__('Specify the Content Block to use as a footer content.', 'lerp') ,
			'std' => '',
			'type' => 'custom-post-type-select',
			'section' => 'lerp_footer_section',
			'post_type' => 'lerpblock',
		...
		array(
			...
			'label' => '<i class="fa fa-copyright"></i> ' . esc_html__('Copyright area', 'lerp') ,
			'type' => 'textblock-titled',
			'class' => 'section-title',
			'section' => 'lerp_footer_section',
		) ,
		array(
			...
			'label' => esc_html__('Automatic copyright text', 'lerp') ,
			'desc' => esc_html__('Activate to use an automatic copyright text.', 'lerp') ,
			'std' => 'on',
			'type' => 'on-off',
			'section' => 'lerp_footer_section',
		) ,
		array(
			...
			'label' => esc_html__('Custom copyright text', 'lerp') ,
			'desc' => esc_html__('Insert a custom text for the footer copyright area.', 'lerp') ,
			'type' => 'textarea',
			'section' => 'lerp_footer_section',
			'operator' => 'or',
		...
		array(
			...
			'label' => esc_html__('Content alignment', 'lerp') ,
			'desc' => esc_html__('Specify the footer copyright text alignment.', 'lerp') ,
			'std' => 'left',
			'type' => 'select',
			'section' => 'lerp_footer_section',
			'choices' => array(
		...
		array(
			...
			'settings' => array(
				...
				array(
					...
					'desc' => esc_html__('Activate to hide the social icon in the menu (if the social connections in the menu is active).', 'lerp') ,
					'std' => 'off',
					'type' => 'on-off',
					'condition' => '',
					'operator' => 'and'
		...
		array(
			...
			'desc' => sprintf( wp_kses(__( 'To use Lerp custom styled Google Maps you need to create <a href="%s" target="_blank">here the Google API KEY</a> and paste it in this field.', 'lerp' ), array( 'a' => array( 'href' => array(),'target' => array() ) ) ), 'https://developers.google.com/maps/documentation/javascript/get-api-key#get-an-api-key' ),
			'type' => 'text',
		...
		array(
			...
			'desc' => esc_html__('Activate to take advantage of the Adaptive Images system. Adaptive Images detects your visitor\'s screen size and automatically delivers device appropriate re-scaled images.', 'lerp') ,
			'std' => 'on',
			'type' => 'on-off',
		...
		array(
			...
			'desc' => esc_html__('Activate to load the adaptive images asynchronously, this will improve the loading performance and it\'s necessary if using an aggresive caching system.', 'lerp') ,
			'std' => 'off',
			'type' => 'on-off',
			'section' => 'lerp_performance_section',
			'operator' => 'or',
		...
		array(
			...
			'desc' => esc_html__('Activate to use the current mobile orientation width (portrait or landscape) instead of the max device\'s width (landscape).', 'lerp') ,
			'std' => 'off',
			'type' => 'on-off',
			'section' => 'lerp_performance_section',
			'operator' => 'and',
		...
		array(
			...
			'desc' => esc_html__('Activate to limit the pixel density to 2 when generating the most appropriate image for high pixel density displays.', 'lerp') ,
			'std' => 'off',
			'type' => 'on-off',
			'section' => 'lerp_performance_section',
			'operator' => 'and',
		...
		array(
			...
			'desc' => esc_html__('Enter all the image sizes you want use for the Adaptive Images system. NB. The values needs to be comma separated.', 'lerp') ,
			'type' => 'text',
		...
		array(
			...
			'desc' => esc_html__('Activate the enhanced .htaccess, this will improve the web site\'s performance and security.', 'lerp') ,
			'std' => 'off',
			'type' => 'on-off',
			'section' => 'lerp_performance_section',
			'condition' => '',
		...
		array(
			...
			'desc' => esc_html__('Activate this to switch to production mode, the CSS and JS will be cached by the browser and the JS minified.', 'lerp') ,
			'std' => 'off',
			'type' => 'on-off',
			'section' => 'lerp_performance_section',
			'condition' => '',
		...
		array(
			...
			'desc' => esc_html__('Activate to redirect all the website calls to a specific page. NB. This can only be visible when the user is not logged in.', 'lerp') ,
			'std' => 'off',
			'type' => 'on-off',
		...
		array(
			...
			'desc' => esc_html__('Specify the redirect page. NB. This page will be presented without menu and footer.', 'lerp') ,
			'type' => 'page_select',
			'section' => 'lerp_redirect_section',
			'post_type' => 'page',
	...
	if (class_exists('WooCommerce'))
	{
		...
		$woo_section = array(
			...
			array(
				...
				'title' => '<span class="smaller"><i class="fa fa-paper"></i> ' . esc_html__('Product', 'lerp') . '</span>',
				'group' => esc_html__('Single', 'lerp'),
			) ,
			array(
				'id' => 'lerp_product_index_section',
				'title' => '<span class="smaller"><i class="fa fa-archive2"></i> ' . esc_html__('Products', 'lerp') . '</span>',
				'group' => esc_html__('Archives', 'lerp'),
			) ,
		);
