...
function ot_type_background($args = array())
{
    ...
    if ( in_array('background-color', $ot_recognized_background_fields) ) {
        ...
        foreach ( $colors_array as $key => $value ) {
            if ( isset($value['disabled']) ) echo '<option value="" disabled>' . esc_attr($value['label']) . '</option>';
                ...
                echo '<option class="' . esc_attr($value['value']) . '" value="' . esc_attr($value['value']) . '" ' . selected($background_color,
                        $value['value'],
                        false) . '>' . esc_attr($value['label']) . '</option>';
            }
        }
    ...
    if ( in_array('background-repeat', $ot_recognized_background_fields) ) {
        ...
        foreach ( ot_recognized_background_repeat($field_id) as $key => $value ) {
            ...
            echo '<option value="' . esc_attr($key) . '" ' . selected($background_repeat,
                    $key,
                    false) . '>' . esc_attr($value) . '</option>';
        }
    ...
    if ( in_array('background-attachment', $ot_recognized_background_fields) ) {
        ...
        foreach ( ot_recognized_background_attachment($field_id) as $key => $value ) {
            ...
            echo '<option value="' . esc_attr($key) . '" ' . selected($background_attachment,
                    $key,
                    false) . '>' . esc_attr($value) . '</option>';
        }
    ...
    if ( in_array('background-position', $ot_recognized_background_fields) ) {
        ...
        foreach ( ot_recognized_background_position($field_id) as $key => $value ) {
            ...
            echo '<option value="' . esc_attr($key) . '" ' . selected($background_position,
                    $key,
                    false) . '>' . esc_attr($value) . '</option>';
        }
        ...
        if ( is_array($choices) && !empty($choices) ) {
            ...
            foreach ( (array)$choices as $choice ) {
                if ( isset($choice['value']) && isset($choice['label']) ) {
                    echo '<option value="' . esc_attr($choice['value']) . '"' . selected((isset($field_value['background-size']) ? $field_value['background-size'] : ''),
                            $choice['value'],
                            false) . '>' . esc_attr($choice['label']) . '</option>';
                }
            }
            ...
            echo '<input type="text" name="' . esc_attr($field_name) . '[background-size]" id="' . esc_attr($field_id) . '-size" value="' . (isset($field_value['background-size']) ? esc_attr($field_value['background-size']) : '') . '" class="widefat ot-background-size-input option-tree-ui-input ' . esc_attr($field_class) . '" placeholder="' . esc_html__('background-size',
                    'lerp') . '" />';
        }
    }
    ...
    if ( in_array('background-image', $ot_recognized_background_fields) ) {
        ...
        /* add media button */
        echo '<a href="javascript:void(0);" class="ot_upload_media option-tree-ui-button button button-secondary light" rel="' . $post_id . '" title="' . esc_html__('Add Media',
                'lerp') . '"><span class="icon fa fa-plus2"></span>' . esc_html__('Add Media', 'lerp') . '</a>';
        ...
        if ( isset($field_value['background-image']) && $field_value['background-image'] !== '' ) {
            ...
            if ( isset($post) ) {
                ...
                else if ( preg_match('/\.(?:jpe?g|png|gif|ico)$/i',
                        $post->guid) || $post->post_mime_type == 'image/url' ) echo '<div class="option-tree-ui-image-wrap"><img src="' . esc_url($post->guid) . '" alt="" /></div>';
            ...
            echo '<a href="javascript:(void);" class="option-tree-ui-remove-media option-tree-ui-button button button-secondary light" title="' . esc_html__('Remove Media',
                    'lerp') . '"><span class="icon fa fa-minus2"></span>' . esc_html__('Remove Media', 'lerp') . '</a>';
...
if ( !function_exists('ot_list_item_view') ) {
    ...
    <div class="option-tree-setting">
      ...
      <div class="button-section">
        <a href="javascript:void(0);" class="option-tree-setting-edit option-tree-ui-button button" title="' . esc_html__('Edit',
                'lerp') . '">
          <span class="icon ot-icon-pencil"></span>' . esc_html__('Edit', 'lerp') . '
        </a>
        <a href="javascript:void(0);" class="option-tree-setting-remove option-tree-ui-button button button-secondary light" title="' . esc_html__('Delete',
                'lerp') . '">
      ...
      <div class="option-tree-setting-body">';
        ...
        foreach ( $settings as $field ) {
            ...
            if ( isset($field['std']) ) {
                $field_value = ot_filter_std_value($field_value,
                    ((isset($field['class']) && $field['class'] === 'unique_id') ? $field['std'] . big_rand() : $field['std']));
            }
    ...
    function ot_type_lerp_color($args = array())
    {
        ...
        foreach ( $colors_array as $key => $value ) {
            if ( isset($value['disabled']) ) echo '<option value="" disabled>' . esc_attr($value['label']) . '</option>';
                ...
                echo '<option class="' . esc_attr($value['value']) . '" value="' . esc_attr($value['value']) . '" ' . selected($field_value,
                        $value['value'],
                        false) . '>' . esc_attr($value['label']) . '</option>';
            }
        }
...
if ( !function_exists('lerp_create_dynamic_css') ) {
    function lerp_create_dynamic_css()
    {
        ...
        require(get_template_directory() . '/inc/style-custom.css.php'); // Generate CSS
        ...
        if ( !$wp_filesystem->put_contents($css_dir . 'style-custom' . $ot_id . '.css',
                $css,
                $mod_file) || !$wp_filesystem->put_contents(get_template_directory() . '/core/assets/css/admin-custom' . $ot_id . '.css',
                $admin_css,
                $mod_file) )
            return array(
                'custom' => $css,
...
if ( !function_exists('lerp_process_toggle_ajax_theme_panel') ) :
    ...
    {
        if ( isset($_POST['toggle_ajax']) && isset($_POST['lerp_toggle_ajax_theme_panel_nonce']) && wp_verify_nonce($_POST['lerp_toggle_ajax_theme_panel_nonce'],
                'lerp_toggle_ajax_theme_panel') ) {
