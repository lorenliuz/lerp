...
class Lerp_Font
{
    ...
    function __construct()
    {
        ...
        if ( isset($_REQUEST['action']) && $_REQUEST['action'] === 'lerp_font_download_font' && isset($_REQUEST['fileaccess']) && $_REQUEST['fileaccess'] === 'other' ) {
            $this->download_font();
            die();
        }
        ...
        add_action('wp_ajax_lerp_font_download_font', array(&$this, 'download_font'));
        add_action('wp_ajax_lerp_font_delete_download', array(&$this, 'delete_download'));
    ...
    {
        $this->fonts_page_name = add_submenu_page('lerp-system-status',
            'Font Stacks',
            'Font Stacks',
            'edit_theme_options',
            'lerp-font-stacks',
    ...
    function add_admin_scripts($hook)
    {
        ...
        if ( $this->fonts_page_name == $hook ) {
            ...
            wp_enqueue_script('uf-preview',
                get_template_directory_uri() . '/inc/font-system/js/jquery.colorbox.min.js',
                array('jquery'),
                LERP_VERSION);
            wp_enqueue_script('uf-js',
                get_template_directory_uri() . '/inc/font-system/js/admin.js',
                array('jquery', 'uf-preview'),
                LERP_VERSION);
            wp_enqueue_style('uf-admin',
                get_template_directory_uri() . '/inc/font-system/css/admin.css',
                array(),
                LERP_VERSION);
            wp_enqueue_style('uf-preview',
                get_template_directory_uri() . '/inc/font-system/css/colorbox.css',
                array(),
                LERP_VERSION);
            ...
            foreach ( $font_stack as $font ) {
                if ( isset($font['source']) && $font['source'] == 'Google Web Fonts' ) {
                    ...
                    wp_enqueue_style('uf-google-font-' . $google_font_counter,
                        '//fonts.googleapis.com/css?family=' . urlencode($font_family),
                        array(),
                        LERP_VERSION);
                }

                if ( isset($font['source']) && $font['source'] == 'Font Squirrel' ) {
                    if ( @file_exists(trailingslashit($this->font_directory) . 'lerp-font.css') ) wp_enqueue_style('uf-font-squirrel',
                        trailingslashit($this->font_directory_url) . 'lerp-font.css',
                        array(),
                        LERP_VERSION);
                }
            }
    ...
    function print_direct_scripts()
    {
        ...
        global $lerp_font_typekit_text, $lerp_font_typekit_error;
        ...
        if ( isset($this->options['typekit_kit_ID']) && $this->options['typekit_kit_ID'] !== '' ) {
            if ( isset($this->options['typekit_api_key']) ) {
                ...
                $lerp_font_typekit_text = $lerp_font_typekit_error = '';
                if ( !is_wp_error($response) ) {
                    $response = wp_remote_retrieve_body($response);
                    $response = json_decode($response);

                    if ( isset($response->kit) ) {
                        $lerp_font_typekit_text .= "<div>";
                        $lerp_font_typekit_text .= "<strong>" . esc_html__('Your kits:', 'lerp') . " </strong>";
                        $kit_string = "";
                        $family_string = "";
                        $kit_position = 0;
                        ...
                        if ( $kit_string != '' ) {
                            ...
                            $lerp_font_typekit_text .= $kit_string;
                        }
                        $lerp_font_typekit_text .= "</div>";
                        $lerp_font_typekit_text .= $family_string;
                        $script .= "<script type='text/javascript'>try{Typekit.load();}catch(e){}</script>\n";
                    } else {
                        $lerp_font_typekit_text .= esc_html__('No kits found for the API key you provided.',
                            'lerp');
                    }
                } else {
                    $lerp_font_typekit_error .= $this->connection_failed('Typekit', false);
                }
                if ( !is_admin() ) {
                    $echo_script = false;
        ...
        } else $lerp_font_typekit_text .= esc_html__('No kits ID inserted.', 'lerp');
    }
                    ...
                    switch ($font['source']) {
                        ...
                        case 'Font Squirrel':
                            if ( isset($font['variantselectors']) && trim($font['variantselectors']) != '' ) {
                                ...
                                $len = count($variants);
                                for ( $i = 0; $i < $len; $i++ ) {
                                    if ( in_array($variants[$i],
                                            $selected_variants) && trim($variant_selectors[$i]) != '' ) {
            ...
            foreach ( $this->font_stack as $font ) {
                if ( isset($font['source']) ) {
                    switch ($font['source']) {
                        ...
                        case 'Fontdeck':
                            if ( isset($this->options['fontdeck_project']) && trim($this->options['fontdeck_project']) != '' && !$fontdeck_included ) {
                                ...
                                if ( !is_wp_error($response) ) {
                                    $response = wp_remote_retrieve_body($response);
                                    $response = json_decode($response);
                                    wp_enqueue_style('lerp-font-fontdeck', $response->cssurl, array(), UNCODE_VERSION);
                        ...
                        case 'Font Squirrel':
                            if ( @file_exists(trailingslashit($this->font_directory) . 'lerp-font.css') && !(isset($this->options['fontsquirrel_combine']) && $this->options['fontsquirrel_combine'] == 'dont-combine') ) {
                                wp_enqueue_style('uf-font-squirrel',
                                    trailingslashit($this->font_directory_url) . 'lerp-font.css',
                                    array(),
                                    UNCODE_VERSION);
                            } else {
                                ...
                                if ( @file_exists(trailingslashit($this->font_directory) . $font['stub']) &&
                                    ...
                                    wp_enqueue_style('lerp-font-squirrel-' . $font['stub'],
                                        trailingslashit($this->font_directory_url) . $font['stub'] . '/stylesheet.css',
                                        array(),
                                        UNCODE_VERSION);
                                }
                            }
    ...
    function admin_init()
    {
        register_setting('lerp_font_options-fonts', 'uncode_font_options', array(&$this, 'validate_options'));
        register_setting('lerp_font_options-settings', 'uncode_font_options', array(&$this, 'validate_options'));
    }
    ...
    function after_form_submit()
    {
        ...
        if ( !@file_exists($this->font_directory) ) {
            if ( !wp_mkdir_p($this->font_directory) ) {
                echo '<div class="uncode-ui-notice uncode-ui-notice--error">' . sprintf(wp_kses(__('Failed to create directory <code>%1$s</code>. Please make sure that you have permissions to create the folder.',
                        'uncode'),
                        array('code' => array())),
                        esc_url($this->font_directory)) . '</div>';
                return;
            }
        }
        ...
        if ( !(isset($this->options['fontsquirrel_combine']) && $this->options['fontsquirrel_combine'] == 'dont-combine') ) {
            ...
            if ( isset($wp_filesystem) && !$wp_filesystem->put_contents(trailingslashit($this->font_directory) . 'lerp-font.css',
                    $css,
                    $this->mod_file) ) {
                echo '<div class="uncode-ui-notice uncode-ui-notice--error">' . esc_html__('Failed to save file lerp-font.css. Please check your folder permissions.',
                        'lerp') . '</div>';
                return;
            }
        }

        echo '<div class="uncode-ui-notice uncode-ui-notice--success">' . esc_html__('Fonts saved.',
                'lerp') . '</div>';
    }

    function add_meta_boxes()
    {
        add_meta_box('lerp-font-google',
            'Google Fonts',
            array(&$this, 'select_from_google_fonts'),
            $this->fonts_page_name,
            'column1');
        add_meta_box('lerp-font-typekit',
            'Typekit',
            array(&$this, 'select_from_typekit'),
            $this->fonts_page_name,
            'column1');
        add_meta_box('lerp-font-fontdeck',
            'Fontdeck',
            array(&$this, 'select_from_fontdeck'),
            $this->fonts_page_name,
            'column1');
        add_meta_box('lerp-font-font-squirrel',
            'Font Squirrel',
            array(&$this, 'select_from_font_squirrel'),
            $this->fonts_page_name,
            'column1');
    }
                    ...
                    <div class="ui-tabs">
                        <ul class="ui-tabs-nav uncode-admin-panel__left">
                            ...
                            <li id="tab_font_source_settings" class="ot-section-label">
                                <a href="#section_font_source_settings"><?php esc_html_e('Font Libraries',
                                        'uncode'); ?></a>
                            </li>
                        </ul>
                            ...
                            <div id="post-body">
                                <div id="post-body-content">
                                    <div id="section_font_stack">
                                        <h2 class="uncode-admin-panel__heading label"><?php esc_html_e('Font Imports',
                                                'uncode'); ?></h2>

                                        <form method="post" action="options.php">
                                            <input type="submit" name="Submit"
                                                   class="button button-primary uncode-ui-button"
                                                   value="<?php esc_html_e('Save Font Imports', 'uncode'); ?>"/>
                                            <?php $this->show_stack(); ?>
                                            ...
                                            ?>
                                            <input type='hidden' id="uf-preview-link"
                                                   value="<?php echo get_template_directory_uri() . '/inc/font-system/preview.php'; ?>"/>
                                        </form>
                                    </div><!-- #section_font_stack -->

                                    <div id="section_font_source_settings" class='font-source-settings'>
                                        <h2 class="uncode-admin-panel__heading label"><?php esc_html_e('Font Libraries',
                                                'uncode'); ?></h2>
                                        ...
                                        <form method="post" action="options.php">
                                            ...
                                            <input type="submit" name="Submit"
                                                   class="button button-primary uncode-ui-button"
                                                   value="<?php esc_html_e('Save Font Libraries', 'uncode'); ?>"/>

                                            <div class="font-sources__list">

                                                <div id="font-source-typekit" class="font-source">
                                                    <div class="uncode-info-box">
                                                        <h4 class="font-source__title"><?php esc_html_e('Typekit',
                                                                'uncode'); ?></h4>
                                                        <p class="font-source__description"><?php printf(wp_kses(__('You need a <a href="%1$s">Typekit API key</a> to access your Typekit fonts.',
                                                                'uncode'),
                                                                array('a' => array('href' => array()))),
                                                                esc_url('https://typekit.com/account/tokens')); ?></p>

                                                        <div class="font-source__field">
                                                            <label class="font-source__label">
                                                                <strong><?php esc_html_e('Enter your API Key',
                                                                        'uncode'); ?>:</strong>
                                                                <input class="font-source__input widefat" type="text"
                                                                       id="typekit_api_key"
                                                                       name="uncode_font_options[typekit_api_key]"
                                                                       value="<?php if ( isset($this->options['typekit_api_key']) ) echo esc_attr($this->options['typekit_api_key']); ?>"/>
                                                            </label>
                                                        </div>

                                                        <div class="font-source__field">
                                                            <label class="font-source__label">
                                                                <strong><?php esc_html_e('Enter your Kit ID',
                                                                        'uncode'); ?>:</strong>
                                                                <input class="font-source__input widefat" type="text"
                                                                       id="typekit_kit_ID"
                                                                       name="uncode_font_options[typekit_kit_ID]"
                                                                       value="<?php if ( isset($this->options['typekit_kit_ID']) ) echo esc_attr($this->options['typekit_kit_ID']); ?>"/>
                                                            </label>
                                                        </div>
                                                ...
                                                <div id="font-source-fontdeck" class="font-source">
                                                    <div class="uncode-info-box">
                                                        <h4 class="font-source__title"><?php esc_html_e('Fontdeck',
                                                                'uncode'); ?></h4>
                                                        <p class="font-source__description"><?php printf(wp_kses(__('You will need to create a <a href="%1$s" target="_blank" tabindex="-1">Fontdeck</a> Project to access your fonts from Fontdeck.',
                                                                'uncode'),
                                                                array(
                                                                    'a' => array(
                                                                        'href' => array(),
                                                                ...
                                                                esc_url('http://fontdeck.com')); ?></p>

                                                        <div class="font-source__field">
                                                            <label class="font-source__label">
                                                                <strong><?php esc_html_e('Enter your project id:',
                                                                        'uncode'); ?></strong>
                                                                <input class="font-source__input widefat" type="text"
                                                                       id="fondeck_project"
                                                                       name="uncode_font_options[fontdeck_project]"
                                                                       value="<?php if ( isset($this->options['fontdeck_project']) ) echo esc_attr($this->options['fontdeck_project']); ?>"/>
                                                            </label>
                                                        </div>
                                                    </div><!-- .uncode-info-box -->
                                                </div><!-- .font-source -->
                                                ...
                                                <div id="font-source-squirrel" class="font-source">
                                                    <div class="uncode-info-box">
                                                        <h4 class="font-source__title"><?php esc_html_e('Font Squirrel',
                                                                'uncode'); ?></h4>
                                                        <div class="font-source__field">
                                                            <p>
                                                                <strong><?php esc_html_e('Pull fonts from Font Squirrel?',
                                                                        'uncode'); ?></strong></p>
                                                            <label class="font-source__label">
                                                                <input class="font-source__input" type="radio"
                                                                       name="uncode_font_options[fontsquirrel_pull]" <?php if ( isset($this->options['fontsquirrel_pull']) ) {
                                                                    checked($this->options['fontsquirrel_pull'],
                                                                        'pull');
                                                                } else {
                                                                    echo 'checked';
                                                                } ?> value='pull'/> <?php esc_html_e('Pull',
                                                                    'uncode'); ?>
                                                            </label>
                                                            <label class="font-source__label">
                                                                <input class="font-source__input" type="radio"
                                                                       name="uncode_font_options[fontsquirrel_pull]" <?php if ( isset($this->options['fontsquirrel_pull']) ) checked($this->options['fontsquirrel_pull'],
                                                                    'dont-pull'); ?>
                                                                       value='dont-pull'/> <?php esc_html_e('Don\'t Pull',
                                                                    'uncode'); ?>
                                                            </label>
                                                        </div>
                                                        ...
                                                        <div class="font-source__field">
                                                            <p>
                                                                <strong><?php esc_html_e('Combine Font Squirrel CSS files?',
                                                                        'uncode'); ?></strong></p>
                                                            <label class="font-source__label">
                                                                <input class="font-source__input" type="radio"
                                                                       name="uncode_font_options[fontsquirrel_combine]" <?php if ( isset($this->options['fontsquirrel_combine']) ) {
                                                                    checked($this->options['fontsquirrel_combine'],
                                                                        'combine');
                                                                } else {
                                                                    echo 'checked';
                                                                } ?> value='combine'/> <?php esc_html_e('Combine',
                                                                    'uncode'); ?>
                                                            </label>
                                                            <label class="font-source__label">
                                                                <input class="font-source__input" type="radio"
                                                                       name="uncode_font_options[fontsquirrel_combine]" <?php if ( isset($this->options['fontsquirrel_combine']) ) checked($this->options['fontsquirrel_combine'],
                                                                    'dont-combine'); ?>
                                                                       value='dont-combine'/> <?php esc_html_e('Don\'t Combine',
                                                                    'uncode'); ?>
                                                            </label>
                                                        </div>
                                                    </div><!-- .font-source -->
    ...
    {
        global $lerp_font_fontdeck_text, $lerp_font_fontdeck_error;
        $lerp_font_fontdeck_text = $lerp_font_fontdeck_error = '';
        $css_url = '';
        if ( isset($this->options['fontdeck_project']) && trim($this->options['fontdeck_project']) != '' ) {
            ...
            if ( !is_wp_error($response) ) {
                ...
                if ( isset($response->fonts) && count($response->fonts) > 0 ) {
                    ...
                    $lerp_font_fontdeck_text .= sprintf(esc_html__('The following fonts were found for project %1$s:',
                        'lerp'),
                        $project);
                    $lerp_font_fontdeck_text .= "<div id='uf-fd-$project-fonts' class='uf-fonts-for uf-group-key-position-1'>";
                    $lerp_font_fontdeck_text .= "<ul>";
                    foreach ( $fonts as $font ) {
                        $lerp_font_fontdeck_text .= $this->create_font_line_item(
                            'Fontdeck',
                            'fd',
                            $font,
                            array(
                    ...
                    $lerp_font_fontdeck_text .= "</ul>";
                    $lerp_font_fontdeck_text .= "</div>";
                } else {
                    $lerp_font_fontdeck_text .= sprintf(esc_html__('No fonts found for project %1$s on %2$s. Make sure your project id is correct and this domain is added to it in your Fontdeck account.',
                        'lerp'),
                        $project,
                        $domain);
                }
            } else {
                $lerp_font_fontdeck_error .= $this->connection_failed('Fontdeck', false);
            }
        }
        return $css_url;
    }
    ...
    function select_from_google_fonts()
    {
        ...
        $file = get_template_directory() . '/inc/font-system/js/google_fonts.json';
        $can_read_file = true;
    ...
    function select_from_font_squirrel()
    {
        if ( isset($this->options['fontsquirrel_pull']) && $this->options['fontsquirrel_pull'] == 'dont-pull' ) {
            printf(wp_kses(__('You have chosen not to pull fonts from <a href="%1$s">Font Squirrel</a>. To change your settings <a href="%1$s">click here</a>.',
                'uncode'),
                array('a' => array('href' => array()))),
                '#font-source-squirrel');
            return;
        }
        echo '<p>' . sprintf(wp_kses(__('Fonts will be downloaded from <a href="%1$s">Font Squirrel</a> to <code>%2$s</code>. Only downloaded fonts are available for addition to the stack.',
                'uncode'),
                array('a' => array('href' => array()), 'code' => array())),
                'http://fontsquirrel.com',
                esc_url($this->font_directory)) . '</p>';
    ...
    {
        if ( !isset($this->options['fontdeck_project']) || trim($this->options['fontdeck_project']) == '' ) {
            printf(wp_kses(__('Please enter <a href="%1$s">your Fontdeck project id</a> to see the available Fontdeck fonts.',
                'uncode'),
                array('a' => array('href' => array()))),
                '#font-source-fontdeck');
            return;
        }

        global $lerp_font_fontdeck_text, $lerp_font_fontdeck_error;
        if ( trim($lerp_font_fontdeck_text) != '' ) {
            echo do_shortcode($lerp_font_fontdeck_text);
        } else if ( trim($lerp_font_fontdeck_error) != '' ) {
            echo do_shortcode($lerp_font_fontdeck_error);
        }
    }
    ...
    function select_from_typekit()
    {
        if ( !isset($this->options['typekit_api_key']) || trim($this->options['typekit_api_key']) == '' ) {
            printf(wp_kses(__('Please enter <a href="%s" target="_blank" tabindex="-1">your Typekit API Key</a> to see the available Typekit fonts.',
                'uncode'),
                array('a' => array('href' => array()))),
                '#font-source-typekit');
            return;
        ...
        global $lerp_font_typekit_text, $lerp_font_typekit_error;
        if ( $lerp_font_typekit_text != '' ) {
            echo do_shortcode($lerp_font_typekit_text);
        } else if ( $lerp_font_typekit_error != '' ) {
            echo do_shortcode($lerp_font_typekit_error);
        }
    }

    function select_from_fonts_com()
    {
        printf(wp_kses(__('You need an <a href="%1$s">authentication key</a> to use fonts from Fonts.com.', 'uncode'),
            array('a' => array('href' => array()))),
            esc_url('https://webfonts.fonts.com/en-US/Account/AccountInformation'));
    }
    ...
    { ?>
        <div class="uf-font-container">
            ...
            if ( $number_of_fonts > 0 ) { ?>
                ...
                <div id="uf-font-details" class="uf-font-details">
                    <h2>Add Fonts</h2>
                    <p><?php printf(wp_kses(__('You have no fonts in your stack. Please add a font first from the sources below. If you don\'t see any fonts below, make sure you have set up the <a href="%1$s">Font Sources</a> correctly.',
                            'uncode'),
                            array('a' => array('href' => array()))),
                            '#font-source-typekit'); ?></p>
                </div>
                <?php
            }
            ?>

            <input type='hidden' id="font_stack" name="uncode_font_options[font_stack]"
                   value="<?php echo esc_attr($this->font_stack_string); ?>"/>
        </div>
        <?php
    }
    ...
    function create_font_line_item($source_system, $source_system_prefix, $font, $args = array())
    {
        ...
        $preview = "<a href='" . esc_url($getSourceUrl) . "' target='_blank' class='uf-launch-preview' title='" . esc_html__('Preview',
                'uncode') . "'><i class='fa fa-search3'></i></a>";
        ...
        if ( $source_system == 'Font Squirrel' ) {
            if ( @file_exists($this->font_directory) && @file_exists(trailingslashit($this->font_directory) . $font->{$args['stub']}) ) {
                $download = '';
                $delete_download = "<a href='#' class='uf-delete-download uf-delete-download-$source_system_prefix' title='" . esc_html__('Delete Download',
                        'uncode') . "'><i class='fa fa-cross'></i></a>";
                $variant_information = $this->font_squirrel_get_font_information($font->{$args['stub']});
                if ( isset($variant_information['variants']) ) $variant_text = implode(',',
                    $variant_information['variants']);
                $variant_files_text = implode(',', $variant_information['files']);
                $family_id_text = implode(',', array_unique($variant_information['family_ids']));
            } else {
                $download = "<a href='#' class='uf-download-font uf-download-font-$source_system_prefix' title='" . esc_html__('Download',
                        'uncode') . "'><i class='fa fa-arrow-down2'></i></a>";
                $add = '';
                $delete_download = '';
            }
        } else {
    ...
    {
        $ret = sprintf(esc_html__('Sorry, there was an error accessing %1$s', 'lerp'), $to_what);
        if ( $echo ) {
     ...
     * @return array
     */
    public function add_more_fonts($fonts,
                                   $key_format = "%stub%",
                                   $value_format = "%family%",
                                   $replace_stub_with_family_if_empty = true,
                                   $add_quotes = "'")
    {
        ...
        foreach ( $this->font_stack as $font ) {
            if ( $font['source'] != 'Font Squirrel' ) {
                $mod_key = $this->substitute_font_parameters($font,
                    $key_format,
                    $replace_stub_with_family_if_empty,
                    $add_quotes);
                $mod_value = $this->substitute_font_parameters($font,
                    $value_format,
                    $replace_stub_with_family_if_empty,
                    $add_quotes);
    ...
    function substitute_font_parameters($font, $lexed, $replace_stub_with_family_if_empty, $add_quotes)
    {
        ...
        $parsed = ($replace_stub_with_family_if_empty && trim($stub) == '') ? str_replace("%stub%",
            $family,
            $parsed) : str_replace("%stub%", $stub, $parsed);
//		$parsed = str_replace("%stub%", $font['stub'], $parsed);
    ...
    function quotify_family($family, $add_quotes)
    {
        ...
        foreach ( $family_parts as $part ) {
            ...
            } else if ( (substr($part, 0, 1) == '"' && $add_quotes == "'") || (substr($part,
                        0,
                        1) == "'" && $add_quotes == '"') ) {
    ...
    function extend_tinymce_dropdown($init_array)
    {
        ...
        foreach ( $this->font_stack as $font ) {
            if ( @file_exists(trailingslashit($this->font_directory) . 'lerp-font.css') ) $content_css[] = trailingslashit($this->font_directory_url) . 'lerp-font.css';
        ...
        $content_css[] = get_template_directory_uri() . '/inc/assets/css/admin-custom.css';
    ...
    function download_font()
    {
        ...
        if ( isset($_REQUEST['font_url']) ) {
            ...
            if ( !@file_exists($this->font_directory) ) {
                if ( !wp_mkdir_p($this->font_directory) ) {
                    echo json_encode(array(
                        'error' => sprintf(esc_html__('Failed to create directory %1$s. Please make sure that you have permissions to create the folder.',
                            'lerp'),
                            esc_url($this->font_directory))
                    ));
                    die();
                }
            }
            ...
            global $lerp_font_file_response;
            $args = array(
                'sslverify' => false,
                'timeout' => 20000,
            );
            if ( !isset($lerp_font_file_response) ) $lerp_font_file_response = wp_remote_request($font_url, $args);

            if ( !is_wp_error($lerp_font_file_response) ) {
                $zip_file = wp_remote_retrieve_body($lerp_font_file_response);
                global $wp_filesystem;
                if ( isset($wp_filesystem) && !$wp_filesystem->put_contents(trailingslashit($this->font_directory) . $zip_file_name,
                        $zip_file,
                        $this->mod_file) ) {
                    echo json_encode(array(
                        'error' => sprintf(esc_html__('Failed to save %1$s to %2$s. Please ensure that the directory exists and is writable.',
                            'lerp'),
                            esc_url($zip_file_name),
                            esc_url($this->font_directory))
                    ));
                    die();
                }
                ...
                echo json_encode(array(
                    'error' => sprintf(esc_html__('Failed to download file to %1$s. Please ensure that the directory exists and is writable.',
                        'lerp'),
                        esc_url($this->font_directory))
                ));
                die();
            ...
            if ( @file_exists(trailingslashit($this->font_directory) . $zip_file_name) && $zip->open(trailingslashit($this->font_directory) . $zip_file_name,
                ...
                if ( !$wp_filesystem->is_dir(trailingslashit($this->font_directory) . $remote_file_info['basename']) ) {
                    $wp_filesystem->mkdir(trailingslashit($this->font_directory) . $remote_file_info['basename'],
                        FS_CHMOD_DIR);
                }
                ...
                for ( $i = 0; $i < $zip->numFiles; $i++ ) {
                    ...
                    $wp_filesystem->put_contents(trailingslashit($this->font_directory) . $remote_file_info['basename'] . '/' . basename($entry),
        ...
        if ( isset($_REQUEST['font_family']) ) {
            ...
            if ( @file_exists($fontkit_zip) ) {
                if ( !@unlink($fontkit_zip) ) {
                    echo json_encode(array(
                        'error' => sprintf(esc_html__('Failed to delete @fontface kit zip %1$s.', 'lerp'),
                            $fontkit_zip)
                    ));
                    die();
                }
